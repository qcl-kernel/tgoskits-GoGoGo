...
- On AArch64, keep the someboot `hv` feature scoped to the EL2 kernel path. For non-`hv` EL1 boot, choose the EL1 arch timer at runtime from `ID_AA64PFR0_EL1`: use CNTP when EL2 is implemented and CNTV when EL2 is unavailable, and keep the FDT timer interrupt index consistent with the selected mode. `CurrentEL` only reports the active exception level and must not be used as the EL2-availability test for a kernel already entered at EL1. Hypervisor guests whose virtual platform exposes the CNTP path while hiding EL2 must explicitly enable `ax-std/el1-phys-timer`; ordinary bare-metal EL1 builds must retain runtime selection.
...
- On AArch64 GICv3, enable `ICH_HCR_EL2.EN` on the vCPU's assigned physical CPU before its first guest entry; do not defer virtual CPU interface initialization until the first device IRQ. Preserve a repeated edge for an INTID whose list register is already `Active` by changing the LR state to `PendingAndActive`. Skipping every INTID already present in an LR loses back-to-back device completions before the guest EOI; only an already `Pending` or `PendingAndActive` interrupt may be coalesced. Execute an `ISB` after writing `ICH_LR_EL2` or enabling `ICH_HCR_EL2.EN`, before returning to guest entry, so the updated virtual interrupt state is architecturally visible.
- Keep local host IRQs saved across the final queued-interrupt drain and the architecture's guest-entry call. Otherwise a producer IPI can be handled after the software queue was observed empty but before guest entry, leaving the queued virtual IRQ stranded with no later VM exit. The architecture entry restores guest interrupt state; this gate protects the host-side entry window only.
