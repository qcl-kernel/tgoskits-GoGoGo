...
- **Network queue runtime SMP order**: physical NIC initialization is an all-or-nothing post-SMP transaction. Probe may publish one-shot `TakenNetDevice` objects before SMP, but `axruntime` must wait until every intended CPU has an online scheduler, local IRQ state, and working synchronous IPI route before selecting affinity domains, pinning queue/protocol executors, or registering device IRQ actions. Register every action disabled with `NonReentrant`, shared-line semantics, and `IrqAffinity::Fixed(owner_cpu)`; enable it only after the matching owner worker completes its affinity and queue-initialization handshakes. A worker-pin, IRQ-route, shared-affinity, or rearm failure must unwind the complete physical network initialization and must not fall back to `IrqAffinity::Any`, remote IRQ continuation, or periodic polling.
...
- **LoongArch QEMU IRQ contract**: the dynamic LoongArch path targets QEMU `virt`/LS7A-style firmware routing through CPU-local timer/IPI lines, EIOINTC, and PCH-PIC. `loongarch-intc-driver` owns the OS-independent IOCSR/MMIO protocols and split controller/CPU-interface endpoints; `somehal` owns firmware parsing, mapping, dynamic domains, `rdrive`, cascade policy, and `ActiveIrq`. `somehal::begin_irq(raw)` receives the CPU interrupt line from `ESTAT.IS`, not an ACPI GSI or PCI vector; only the timer line, IPI line, and EIOINTC cascade line may enter runtime dispatch. Publish the EIO controller, domain, and shutdown-lifetime CPU interface before enabling its CPU-local cascade. Registering an Axvisor virtual IRQ injector must not re-enable that line or reinterpret it as `IrqSource::AcpiGsi(3)`. PCH-PIC owns local trigger/polarity, mask, and HTVEC state through its `rdif_intc::Intc` lock, while its immutable CPU interface derives `EioVector -> PchInput` only from the validated base/count. PCH `set_enabled` is local-only: platform glue performs the existing parent-EIO then local-PCH order, releases one controller lock before acquiring the other, and restores the parent if the local step fails. Do not infer PCH input by subtracting `PCI_INTX_VECTOR_BASE`, do not store ACPI `route.vector = PCI_INTX_VECTOR_BASE + gsi` as an EIO hardware vector, and do not dispatch unknown CPU-local interrupt lines as PCH-PIC IRQs.
- **LoongArch LS2K LIOINTC contract**: follow the AArch64 GIC distributor/CPU-interface ownership split implemented by `loongarch-intc-driver`. The `rdif_intc` controller owns route and W1 enable/disable registers. A separately published shutdown-lifetime CPU interface owns only the ISR mapping, typed parent lines, and shared atomic enabled snapshot; the platform wrapper attaches the dynamic domain after claim. Register the RDIF controller and publish the CPU interface/domain before enabling any parent cascade. Hard IRQ must not call `rdrive::get_list`, take a task-owned controller lock, allocate, or block. Publish enable with Release after the controller's hardware write, and hide a disabled input with AcqRel before disabling it in hardware. LIO completion performs no hardware EOI because the current inputs are level-triggered and deasserted by their device handlers.
- **LoongArch LS2K LIOINTC parent routing**: each hard-IRQ claim must filter the ISR by the triggering parent line's effective input bitmap as well as the enabled snapshot. Inputs selected by overlapping firmware bitmaps follow the programmed first-match route, and inputs omitted from every bitmap belong only to the first configured fallback parent. Simultaneously pending inputs on different parents must never be cross-claimed.
...
- **Axvisor virtual-device planning**: keep VM initialization order architecture-owned. Each architecture builds a `DeviceGraphBuilder` containing its controller, bus, host-replacement, passthrough, firmware-only, and configured virtual-device nodes. Graph nodes retain the exact `Arc<dyn DeviceModel>` used for mandatory `requirements()`, `firmware()`, and `build()` calls; evaluate `firmware()` once when declaring the node and freeze its typed contribution. `DeviceFirmwareSpec::None` means no platform node, while `Interfaces` may be FDT-only, ACPI-only, or support both; reject an empty supported side and reject every device that lacks an interface selected by the platform. Resolve contribution slots to the same final address, IRQ, and controller identity used by runtime, and keep common FDT/ACPI composers free of model, name, ID, fixed-address matches, raw DTS/AML, and fallback guessing. User configuration is `id + model + typed options`, resolved by an explicitly populated, initially empty `ConfiguredDeviceCatalog` whose entries are plain `ConfiguredModelRegistration` function pointers with an owner path; do not add a factory trait, instance wrapper, device-type enum, linker registration, or firmware dyn container. AxVM owns and explicitly registers common devices, including IVC and virtio, while Axvisor only supplies VM parameters. Fixed requests come only from machine policy or normalized host firmware. Every architecture creates its own deterministic plan, registers its interrupt-controller bundle before IRQ/MSI consumers, issues one `ResourceClaimSet` per node, and seals `DeviceRuntime` only after every slot is retained by a lease. FDT/ACPI and runtime read the same `ResolvedDeviceGraph`. MMIO/PIO/SysReg exits perform one runtime lookup and dyn `Device::read/write`; do not restore address/device special cases, downcasts, `find_*` plus second dispatch, device-specific post-access calls, a shared cross-architecture device sequence, a second interrupt fabric, or guessed controller/address/IRQ fallback. Device-specific post-access publication belongs inside the device or a typed controller service injected during `build()`. AArch64 creates one immutable plan before final DTB serialization; the same `ArmVgicConfig` and resolved resources drive VGIC construction, GICR/ITS nodes, and endpoints. Host-derived GICD, GICC/GICR, and ITS windows must be disjoint and non-overflowing. RISC-V retains PLIC hart/context ordering, x86 retains LAPIC/IOAPIC/PIT/APIC-access ordering, and LoongArch retains IOCSR/EXTIOI/PCH-PIC cascading. Keep this contract synchronized with `docs/design/axvisor-resolved-device-graph.md` and `docs/design/arm-vgic-interrupt-topology.md`.
- **Axvisor physical UART passthrough**: the physical UART selected as the host console by firmware remains host-owned and excluded from guest passthrough. `GuestDevices::passthrough` FDT selectors include the named node and its descendants using FDT node-boundary matching, so `/peripherals` includes `/peripherals/serial@...` but not `/peripherals-extra/serial@...`. A selector that includes the host console UART must fail with `HostOwnedDevice`. Explicitly selected non-console physical UARTs retain their MMIO range, IRQ route, and guest FDT node; unselected physical UART nodes are removed when the machine-owned virtual console is installed. If a selected UART uses an interrupt source reserved by an excluded host device, VM configuration must fail instead of retaining the guest FDT node without a physical IRQ route.
- **Axvisor firmware contribution consumption**: resolve every selected FDT/ACPI contribution,
  including interrupt-controller, timer, PCI-host, console, and firmware-transport categories.
  Architecture adapters must consume special entries by typed category and firmware identity,
  reject duplicates or unsupported leftovers, and never select them by a hard-coded graph node ID.
  `InterruptController` contributions carry the runtime `InterruptControllerId`; preserve
  `(controller, input)` through resolution, map it explicitly to an FDT phandle or ACPI GSI
  domain, and reject an unmapped controller instead of binding the default parent or flattening
  the input into a GSI. Keep this contract synchronized with
  `docs/design/axvisor-resolved-device-graph.md`.
