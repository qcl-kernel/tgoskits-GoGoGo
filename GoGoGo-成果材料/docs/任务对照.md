...
| 1.5 可复现（分支/镜像/配置/命令/脚本/结果采集） | ✅ | [复现指南.md](复现指南.md) 全流程命令 |
...
代表性结果（timer jitter，ns；完整表见 [性能数据.md](性能数据.md)）：
...
| 要求材料 | 位置 |
|---|---|
| 设计文档 | [系统设计.md](系统设计.md)（架构/隔离/协议概述）、[更改说明.md](更改说明.md)（配置与修改说明）、`os/axvisor/guests/task3/README.md`（AI 模型选型与部署） |
| 测试文档 | [验证报告.md](验证报告.md)、[性能数据.md](性能数据.md) |
| 源码 PR | 分支 `upstream/pr-new`（源 self 分支 `feat/task123-rock4d-port` 已合入后 rebase）；已推送 origin，与 dev 无冲突 |
| 复现说明 | [复现指南.md](复现指南.md) |
