...
[系统设计.md](系统设计.md)（是什么）、[更改说明.md](更改说明.md)（改了什么）、
[复现指南.md](复现指南.md)（怎么跑）之外的过程叙述。
...
### 3.7 上游工具链 bug：ostool 板级复位命令不生效

- 现象：板级测试的 U-Boot 启动依赖 `.uboot.toml` 里的
  `board_reset_cmd` / `board_power_off_cmd`（串口复位与断电脚本），配置
  写了却从不执行——板子只停在 "Waiting for board on power or reset..."。
- 定位：最小 serde 复现确认——`UbootConfig` 外层与被 `#[serde(flatten)]`
  内联的 `LocalUbootConfig` **重复声明**了这两个字段；flatten 语义下 TOML
  顶层值被外层字段优先占用，而执行复位的 `LocalBackend` 只读 flatten 进来
  的那份（永远是 `None`）。0.27.1 与 0.27.2 均如此。
- 修复：删除 `LocalUbootConfig` 的重复字段，`LocalBackend` 改为在构造时
  从 `UbootConfig` 显式接收这两个命令，并保留旧 Rust API 字段的回退兼容。
  修复以 PR 形式提交上游：
  [drivercraft/ostool#172](https://github.com/drivercraft/ostool/pull/172)
  （fix(ostool): honor board_reset_cmd/board_power_off_cmd from uboot
  config），已获 AI 评审 APPROVE，等待人工合入。
- 本地工作区影响：在修复合入前，本地开发用 `[patch.crates-io]` 指向修复
  分支构建（机器相关配置，不进入仓库）。**这也解释了板级启动偶尔需要手动
  `rk3576-rst` 助推**：自动化复位的缺位正是此 bug 的外在表现之一。
- 教训：工具链层的"配置不生效"要先做最小复现（此处一段 10 行的 serde
  程序）再报上游；flatten + 重名字段是 serde 的经典遮蔽坑，报 PR 时给
  出根因分析比给 patch 更有价值。

### 3.8 busy WFI fastpath 与统一 host-timer 所有权冲突
...
- 验证：8 组合（QEMU×4 + ROCK 4D×4）全部门禁 + 指标完整，正式数据批次
  归档。
- 上游贡献：ostool serde-flatten 遮蔽 bug 的根因定位与修复
  （[drivercraft/ostool#172](https://github.com/drivercraft/ostool/pull/172)，
  已获 AI 评审批准，待人工合入）。
