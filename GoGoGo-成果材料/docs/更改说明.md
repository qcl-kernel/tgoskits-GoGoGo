...
修复上游工具链 bug：ostool 的 `UbootConfig` 与被 `#[serde(flatten)]` 内联
的 `LocalUbootConfig` 重复声明 `board_reset_cmd`/`board_power_off_cmd`，
flatten 语义使 TOML 顶层值被外层占用而 `LocalBackend` 读到的内层份永远为
`None`——板级自动化复位/断电从不执行。删除重复字段并让 `LocalBackend` 从
`UbootConfig` 显式接收命令（保留旧 Rust API 字段回退）。PR：
[drivercraft/ostool#172](https://github.com/drivercraft/ostool/pull/172)
（已获 AI 评审 APPROVE，待人工合入；本地暂以 `[patch.crates-io]` 使用修复
版，该段为机器相关配置不进仓库）。定位过程见
[工程实录 §3.7](工程实录.md)。
