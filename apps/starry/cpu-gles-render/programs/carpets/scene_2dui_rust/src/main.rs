...
fn q8(f: f32) -> i32 { clampi((f * 255.0).round() as i32, 0, 255) }
...
fn run() -> i32 {
    ...
    unsafe {
        ...
        let fill_rect = |gl: &glow::Context, u_loc: Option<&glow::UniformLocation>,
                         x0: f32, y0: f32, x1: f32, y1: f32, r: f32, g: f32, b: f32, a: f32| {
        ...
        let prog = mk(VS_PIX, FS_UNI); ok(true, "pixel-fill program compiles+links");
        ...
        fill_rect(&gl, u_loc.as_ref(), 8.0, 8.0, 16.0, 24.0, 1.0, 0.0, 0.0, 1.0);
        fill_rect(&gl, u_loc.as_ref(), 40.0, 32.0, 48.0, 52.0, 0.0, 1.0, 0.0, 1.0);
        gl.finish(); readback(&gl, &mut buf);
        {
            ...
            for y in 0..H { for x in 0..W {
                ...
                if !peq(&buf, x, y, er, eg, eb, 255, 1) { bad += 1; }
            }}
            ...
            ok(peq(&buf, 44, 40, 0, 255, 0, 255, 1), "rect B interior green");
            ok(peq(&buf, 30, 30, 0, 0, 0, 255, 1), "gap between rects is background");
        }

        // ---- Scene B: analytic rounded-rect ----
        {
            let fs_rr = "#version 310 es\nprecision highp float;\nlayout(location=0) out vec4 o;\n\
                uniform vec4 box;\nuniform float rad;\nuniform vec4 col;\n\
                void main(){ vec2 p=gl_FragCoord.xy; float x0=box.x,y0=box.y,x1=box.z,y1=box.w;\n\
                  ...
                  o=col; }\n";
            let prr = mk(VS_PIX, fs_rr); ok(true, "rounded-rect program compiles+links");
            gl.use_program(Some(prr));
            gl.uniform_2_f32(gl.get_uniform_location(prr, "vp").as_ref(), W as f32, H as f32);
            gl.uniform_4_f32(gl.get_uniform_location(prr, "box").as_ref(), 12.0, 12.0, 52.0, 52.0);
            gl.uniform_1_f32(gl.get_uniform_location(prr, "rad").as_ref(), 8.0);
            gl.uniform_4_f32(gl.get_uniform_location(prr, "col").as_ref(), 1.0, 1.0, 0.0, 1.0);
            gl.clear_color(0.0, 0.0, 0.0, 1.0); gl.clear(glow::COLOR_BUFFER_BIT);
            ...
            gl.vertex_attrib_pointer_f32(0, 2, glow::FLOAT, false, 8, 0);
            gl.draw_arrays(glow::TRIANGLES, 0, 6); gl.finish(); readback(&gl, &mut buf);
            let covered = |x: i32, y: i32| -> bool {
                ...
                if corner { let dx = cx - ccx; let dy = cy - ccy; if (dx * dx + dy * dy).sqrt() > r { return false; } }
                true
            };
            ...
            ok(peq(&buf, 32, 32, 255, 255, 0, 255, 1), "rounded-rect center lit");
            ok(peq(&buf, 12, 12, 0, 0, 0, 255, 1), "rounded-rect clipped corner (12,12) is background");
            ok(peq(&buf, 32, 13, 255, 255, 0, 255, 1), "rounded-rect straight top edge lit");
            gl.delete_program(prr); gl.use_program(Some(prog));
        }

        // ---- Scene C: nine-patch-style scaled border frame ----
        gl.clear_color(0.0, 0.0, 0.0, 1.0); gl.clear(glow::COLOR_BUFFER_BIT);
        fill_rect(&gl, u_loc.as_ref(), 4.0, 4.0, 60.0, 60.0, 0.0, 0.0, 1.0, 1.0);
        fill_rect(&gl, u_loc.as_ref(), 10.0, 10.0, 54.0, 54.0, 0.1, 0.1, 0.1, 1.0);
        gl.finish(); readback(&gl, &mut buf);
        {
            let mut bad = 0;
            for y in 0..H { for x in 0..W {
            ...
            ok(peq(&buf, 5, 32, 0, 0, 255, 255, 1), "nine-patch left border blue");
            ok(peq(&buf, 32, 5, 0, 0, 255, 255, 1), "nine-patch top border blue");
            ok(peq(&buf, 32, 32, q8(0.1), q8(0.1), q8(0.1), 255, 1), "nine-patch hollow interior");
        }
        ...
        let glyph_h: [u8; 8] = [0x00, 0x42, 0x42, 0x7E, 0x42, 0x42, 0x42, 0x00];
        {
            ...
            for r in 0..8 { for c in 0..8 {
                let lit = (glyph_h[r] >> (7 - c)) & 1 != 0; let v = if lit { 255u8 } else { 0u8 };
                let idx = (r * 8 + c) * 4; rgba[idx] = v; rgba[idx + 1] = v; rgba[idx + 2] = v; rgba[idx + 3] = 255;
            }}
            let gtex = gl.create_texture().unwrap(); gl.active_texture(glow::TEXTURE0); gl.bind_texture(glow::TEXTURE_2D, Some(gtex));
            gl.tex_image_2d(glow::TEXTURE_2D, 0, glow::RGBA8 as i32, 8, 8, 0, glow::RGBA, glow::UNSIGNED_BYTE, Some(&rgba));
            gl.tex_parameter_i32(glow::TEXTURE_2D, glow::TEXTURE_MIN_FILTER, glow::NEAREST as i32);
            gl.tex_parameter_i32(glow::TEXTURE_2D, glow::TEXTURE_MAG_FILTER, glow::NEAREST as i32);
            ...
            gl.uniform_2_f32(gl.get_uniform_location(ptex, "vp").as_ref(), W as f32, H as f32);
            ...
            for dy in 0..8 { for dx in 0..8 {
                ...
                let lit = (glyph_h[trow] >> (7 - tcol)) & 1 != 0; let v = if lit { 255 } else { 0 };
                if !peq(&buf, sx, sy, v, v, v, 255, 1) { bad += 1; }
            }}
            ok(bad == 0, "glyph blit: all 64 texels match hardcoded 8x8 'H' bitmap");
            ok(peq(&buf, 21, 23, 255, 255, 255, 255, 1), "glyph crossbar lit (col1,row3)");
            ok(peq(&buf, 23, 20, 0, 0, 0, 255, 1), "glyph row0 blank");
            ok(peq(&buf, 24, 21, 0, 0, 0, 255, 1), "glyph row1 middle blank (0x42)");
            gl.delete_program(ptex); gl.delete_texture(gtex); gl.disable_vertex_attrib_array(1); gl.use_program(Some(prog));
        }

        // ---- Scene E: scissor-clipped fill ----
        gl.clear_color(0.0, 0.0, 0.0, 1.0); gl.clear(glow::COLOR_BUFFER_BIT);
        gl.enable(glow::SCISSOR_TEST); gl.scissor(16, 16, 20, 20);
        fill_rect(&gl, u_loc.as_ref(), 0.0, 0.0, W as f32, H as f32, 1.0, 0.0, 1.0, 1.0);
        gl.disable(glow::SCISSOR_TEST); gl.finish(); readback(&gl, &mut buf);
        {
            let mut bad = 0;
            for y in 0..H { for x in 0..W {
            ...
            ok(peq(&buf, 20, 20, 255, 0, 255, 255, 1), "scissor inside magenta");
            ok(peq(&buf, 40, 40, 0, 0, 0, 255, 1), "scissor outside background");
        }
        ...
        // ---- Scene F: MULTI-LAYER Porter-Duff over compositing ----
        {
            ...
            gl.clear_color(bg[0], bg[1], bg[2], bg[3]); gl.clear(glow::COLOR_BUFFER_BIT);
            gl.enable(glow::BLEND); gl.blend_func(glow::SRC_ALPHA, glow::ONE_MINUS_SRC_ALPHA); gl.blend_equation(glow::FUNC_ADD);
            for l in &layers { fill_rect(&gl, u_loc.as_ref(), l[4], l[5], l[6], l[7], l[0], l[1], l[2], l[3]); }
            gl.disable(glow::BLEND); gl.finish(); readback(&gl, &mut buf);
            let composite = |tx: i32, ty: i32| -> [f32; 4] {
                ...
                for l in &layers {
                    ...
                    if cx >= l[4] && cx < l[6] && cy >= l[5] && cy < l[7] {
                        ...
                        for k in 0..4 { c[k] = src[k] * as_ + c[k] * (1.0 - as_); }
                    }
                }
                c
            };
            ...
            for y in 0..H { for x in 0..W {
                let e = composite(x, y);
                if !peq(&buf, x, y, q8(e[0]), q8(e[1]), q8(e[2]), q8(e[3]), 2) { bad += 1; }
            }}
            ok(bad == 0, "multi-layer over: every pixel matches Porter-Duff over accumulation (incl partial-overlap regions)");
            {
                ...
                for li in &ls { let as_ = li[3]; for k in 0..4 { c[k] = li[k] * as_ + c[k] * (1.0 - as_); } }
                ok(peq(&buf, 32, 32, q8(c[0]), q8(c[1]), q8(c[2]), q8(c[3]), 2), "multi-layer over center pixel matches hand-iterated over");
            }
            {
                let as_ = 0.5f32;
                ...
                ok(peq(&buf, 10, 32, q8(er), q8(eg), q8(eb), q8(ea), 2), "multi-layer over: single-layer region matches one over");
            }
        }
        ...
        fill_rect(&gl, u_loc.as_ref(), 8.0, 8.0, 16.0, 24.0, 1.0, 0.0, 0.0, 1.0);
        gl.finish(); readback(&gl, &mut buf);
        ok(!peq(&buf, 10, 10, 0, 255, 0, 255, 4), "negative control: red rect pixel is NOT green");
        ok(!peq(&buf, 30, 30, 255, 0, 0, 255, 4), "negative control: background is NOT red");
    }
