...
fn mul(a: &M4, b: &M4) -> M4 {
    ...
    for c in 0..4 { for row in 0..4 {
        let mut s = 0.0;
        for k in 0..4 { s += a.m[k * 4 + row] * b.m[c * 4 + k]; }
        r.m[c * 4 + row] = s;
    }}
    r
}
fn mv4(a: &M4, v: &[f32; 4]) -> [f32; 4] {
    let mut o = [0.0f32; 4];
    for row in 0..4 { let mut s = 0.0; for k in 0..4 { s += a.m[k * 4 + row] * v[k]; } o[row] = s; }
    o
}
fn perspective(fovy: f32, aspect: f32, zn: f32, zf: f32) -> M4 {
    let f = 1.0 / (fovy * 0.5).tan(); let mut r = M4 { m: [0.0; 16] };
    r.m[0] = f / aspect; r.m[5] = f;
    r.m[2 * 4 + 2] = (zf + zn) / (zn - zf); r.m[2 * 4 + 3] = -1.0;
    r.m[3 * 4 + 2] = (2.0 * zf * zn) / (zn - zf); r
}
fn translate(x: f32, y: f32, z: f32) -> M4 {
    let mut r = M4 { m: [0.0; 16] };
    r.m[0] = 1.0; r.m[5] = 1.0; r.m[10] = 1.0; r.m[15] = 1.0;
    r.m[3 * 4] = x; r.m[3 * 4 + 1] = y; r.m[3 * 4 + 2] = z; r
}
fn rot_y(a: f32) -> M4 {
    let mut r = M4 { m: [0.0; 16] }; let (c, s) = (a.cos(), a.sin());
    r.m[0] = c; r.m[0 * 4 + 2] = -s; r.m[2 * 4] = s; r.m[2 * 4 + 2] = c; r.m[5] = 1.0; r.m[15] = 1.0; r
}
fn rot_x(a: f32) -> M4 {
    let mut r = M4 { m: [0.0; 16] }; let (c, s) = (a.cos(), a.sin());
    r.m[5] = c; r.m[1 * 4 + 2] = s; r.m[2 * 4 + 1] = -s; r.m[2 * 4 + 2] = c; r.m[0] = 1.0; r.m[15] = 1.0; r
}
...
fn run() -> i32 {
    ...
    unsafe {
        ...
        for i in 0..8 { for k in 0..3 { vc[i][k] = (vp[i][k] + 1.0) * 0.5; } }
        let idx: [u16; 36] = [
            0, 1, 2, 0, 2, 3,
            4, 6, 5, 4, 7, 6,
            0, 4, 5, 0, 5, 1,
        ...
        for i in 0..8 {
            verts[i * 6] = vp[i][0]; verts[i * 6 + 1] = vp[i][1]; verts[i * 6 + 2] = vp[i][2];
        ...
        gl.buffer_data_u8_slice(glow::ELEMENT_ARRAY_BUFFER, core::slice::from_raw_parts(idx.as_ptr() as *const u8, 72), glow::STATIC_DRAW);
        gl.vertex_attrib_pointer_f32(0, 3, glow::FLOAT, false, 24, 0); gl.enable_vertex_attrib_array(0);
        gl.vertex_attrib_pointer_f32(1, 3, glow::FLOAT, false, 24, 12); gl.enable_vertex_attrib_array(1);

        let prog = {
            let v = gl.create_shader(glow::VERTEX_SHADER).unwrap();
            gl.shader_source(v, "#version 310 es\nlayout(location=0) in vec3 p;\nlayout(location=1) in vec3 c;\nout vec3 vc;\nuniform mat4 mvp;\nvoid main(){ gl_Position=mvp*vec4(p,1.0); vc=c; }\n");
            gl.compile_shader(v); if !gl.get_shader_compile_status(v) { ok(false, "vs compile"); }
            let f = gl.create_shader(glow::FRAGMENT_SHADER).unwrap();
            gl.shader_source(f, "#version 310 es\nprecision highp float;\nin vec3 vc;\nlayout(location=0) out vec4 o;\nvoid main(){ o=vec4(vc,1.0); }\n");
            gl.compile_shader(f); if !gl.get_shader_compile_status(f) { ok(false, "fs compile"); }
        ...
        gl.draw_elements(glow::TRIANGLES, 36, glow::UNSIGNED_SHORT, 0); gl.finish(); readback(&gl, &mut buf);
        ok(gl.get_error() == glow::NO_ERROR, "no GL error after cube draw");
        ...
        for i in 0..8 {
            ...
            sx[i] = (ndcx * 0.5 + 0.5) * W as f32; sy[i] = (ndcy * 0.5 + 0.5) * H as f32; sz[i] = ndcz * 0.5 + 0.5;
        }
        ok(sw[0] > 0.0, "reference: all clip.w positive (mesh in front of camera)");
        for t in 0..12 {
            let (a, b, c) = (idx[t * 3] as usize, idx[t * 3 + 1] as usize, idx[t * 3 + 2] as usize);
        ...
        for y in 0..H { for x in 0..W {
            total += 1;
            let gcov = !(px(&buf, x, y, 0) == 0 && px(&buf, x, y, 1) == 0 && px(&buf, x, y, 2) == 0);
            let pi = (y * W + x) as usize;
            ...
            if rcov {
                ...
                let interior = x > 0 && y > 0 && x < W - 1 && y < H - 1
                    && cov_at(&refcov, x, y - 1) && cov_at(&refcov, x, y + 1)
                    && cov_at(&refcov, x - 1, y) && cov_at(&refcov, x + 1, y);
                if peq(&buf, x, y, er, eg, eb, 255, 6) { mmatch += 1; }
                else if interior { interior_bad += 1; }
            }
        }}
        ok(covtotal > 200, "reference: cube covers a substantial area");
        ok(covmatch >= (0.97 * total as f32) as i32, "coverage mask matches GL (>=97% of pixels agree covered/empty)");
        ok(interior_bad == 0, "every interior pixel matches perspective-correct Gouraud reference (tol 6)");
        ok(mmatch >= (0.92 * covtotal as f32) as i32, "92%+ of covered pixels match reference color (edges excluded)");

        // targeted closed-form spot checks
        {
            let vx = (sx[6] - 0.5).round() as i32; let vy = (sy[6] - 0.5).round() as i32;
            if vx >= 1 && vx < W - 1 && vy >= 1 && vy < H - 1 {
                let mut bright = false;
                for dy in -1..=1 { for dx in -1..=1 {
                    let (xx, yy) = (vx + dx, vy + dy);
                    if px(&buf, xx, yy, 0) > 180 && px(&buf, xx, yy, 1) > 180 && px(&buf, xx, yy, 2) > 180 { bright = true; }
                }}
        ...
        ok(peq(&buf, 0, 0, 0, 0, 0, 255, 1) || refcov[0] == 0, "corner (0,0) background consistent");

        {
            let (cxp, cyp) = (W / 2, H / 2); let pi = (cyp * W + cxp) as usize;
            if refcov[pi] != 0 {
                let er = (refc[pi][0] * 255.0).round() as i32;
                let eg = (refc[pi][1] * 255.0).round() as i32;
                let eb = (refc[pi][2] * 255.0).round() as i32;
                ok(peq(&buf, cxp, cyp, er, eg, eb, 255, 8), "center pixel = nearest-face (depth-buffered occlusion) reference color");
            } else {
        ...
        ok(!(px(&buf, 1, 1, 0) == px(&buf, W / 2, H / 2, 0) && px(&buf, 1, 1, 1) == px(&buf, W / 2, H / 2, 1) && px(&buf, 1, 1, 2) == px(&buf, W / 2, H / 2, 2)),
            "negative control: image is not a flat single color (real 3D shading present)");
    }
