...
the target triple (`${triple}-g++` on `PATH` or `/opt/${triple}-cross`, else `zig c++ -target ${triple}`) and
builds each cell with `--sysroot` / OpenCV flags resolved by a **host** `pkgconf` run against the staging
root. Two hazards are handled: Alpine's OpenCV `.so` carry a `.relr.dyn` section that older musl-cross
binutils `ld` rejects (zig's bundled LLD reads it), and Alpine OpenCV uses the libstdc++ GNU C++ ABI
(`std::__cxx11`) - so the `zig c++` path compiles against the **staged** libstdc++ headers and links the
staged `libstdc++.so.6`, matching that ABI exactly. `prebuild.sh` probes a candidate `${triple}-g++` against
`libopencv_core` and only uses it if its `ld` actually links the `.relr.dyn` libs; otherwise it uses `zig
c++`.
