/* opengl_c_full_api.c - OpenGL C compute API carpet on EGL-surfaceless / llvmpipe (GL 4.5 core,
 * no OSMesa): create a surfaceless EGL desktop-GL 4.5 core context, load the GL 4.3 compute entry
 * points via eglGetProcAddress, and exercise the GL compute-shader API surface (shader-compile+link
 * with error paths / SSBO / buffer-base binding / uniforms / elementwise + shared-memory reduction
 * dispatch / memory-barrier / fence-sync / indirect dispatch / persistent write mapping / multi-dim
 * dispatch / program-resource reflection / limits query / injected GL_INVALID_* validation paths) and
 * asserts every operator result against a closed-form reference, with negative controls proving the
 * checker rejects wrong data. Prints "OPENGL_C_FULL_API OK <n>" only when all assertions pass and
 * count == EXPECTED. Runs on-target on every arch (mesa-gl/mesa-egl present 4-arch) the same way as
 * opengl_c_egl - the OSMesa dependency is gone. Software rasterizer (llvmpipe), single-threaded. */
#include "gl_loader.h"
#include <EGL/egl.h>
#include <GL/gl.h>
#include <stdio.h>
...
int main(void){
  ...
  /* --- surfaceless EGL desktop-GL 4.5 core context (same bring-up as opengl_c_egl) --- */
  ...
  int EXPECTED=83, TOTAL=PASS+FAIL;
