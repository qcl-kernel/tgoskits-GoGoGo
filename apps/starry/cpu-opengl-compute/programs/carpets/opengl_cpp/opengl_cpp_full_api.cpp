// opengl_cpp_full_api.cpp - full desktop OpenGL C++ compute API carpet on surfaceless EGL / llvmpipe
// (GL 4.5 core, GL 4.3 compute shaders): exercise the surfaceless-EGL + GL 4.3 compute surface
...
// Surfaceless-EGL variant (GL 4.5 core over llvmpipe, no OSMesa) - same on-target path as opengl_c_egl.
#include <EGL/egl.h>
...
// glEnable/glIsEnabled are core GL 1.0 and linked directly from libGL, but the pared-down
...
// GL 4.3 compute entry points loaded via eglGetProcAddress (no GLEW). Core GL 1.x symbols
// (glGetString/glGetIntegerv/glGetError/glIsBuffer) are resolved by linking libGL directly.
#define GLPROCS(X) \
...
#define LOAD(t,n) n=(t)eglGetProcAddress(#n); if(!n) okall=false;
  GLPROCS(LOAD)
  return okall; }
...
int main(){
  ...
  // --- surfaceless EGL context (GL 4.5 core, no OSMesa) - mirrors opengl_c_egl's on-target path ---
  EGLDisplay dpy=eglGetDisplay(EGL_DEFAULT_DISPLAY); ok(dpy!=EGL_NO_DISPLAY,"eglGetDisplay");
  EGLint emaj=0,emin=0; ok(eglInitialize(dpy,&emaj,&emin),"eglInitialize");
  ok(eglQueryString(dpy,EGL_VENDOR)!=nullptr,"eglQueryString VENDOR");
  ok(eglQueryString(dpy,EGL_CLIENT_APIS)&&std::strstr(eglQueryString(dpy,EGL_CLIENT_APIS),"OpenGL")!=nullptr,"eglQueryString CLIENT_APIS has OpenGL");
  ...
  ok(gl_load(),"load GL 4.3 compute entry points (eglGetProcAddress)");
  ...
  // --- EGL surfaceless context state: eglQueryContext must report the OpenGL client type, the config
  //     id we chose, and (no surface bound) EGL_NONE as the render buffer - a closed-form context check ---
  { EGLint cval=0;
    ok(eglQueryContext(dpy,ctx,EGL_CONTEXT_CLIENT_TYPE,&cval) && cval==EGL_OPENGL_API,"eglQueryContext CONTEXT_CLIENT_TYPE==OPENGL");
    EGLint cid=0; eglGetConfigAttrib(dpy,cfg,EGL_CONFIG_ID,&cid);
    ok(eglQueryContext(dpy,ctx,EGL_CONFIG_ID,&cval) && cval==cid,"eglQueryContext CONFIG_ID matches chosen config");
    ok(eglQueryContext(dpy,ctx,EGL_RENDER_BUFFER,&cval) && cval==EGL_NONE,"eglQueryContext RENDER_BUFFER==EGL_NONE (surfaceless)"); }
  ...
  eglMakeCurrent(dpy,EGL_NO_SURFACE,EGL_NO_SURFACE,EGL_NO_CONTEXT); eglDestroyContext(dpy,ctx); eglTerminate(dpy);

  const int EXPECTED=127; int TOTAL=PASS+FAIL;
