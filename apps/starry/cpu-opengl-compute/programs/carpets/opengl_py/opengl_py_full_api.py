#!/usr/bin/env python3
# opengl_py_full_api.py - full desktop-GL Python (PyOpenGL) compute API carpet on EGL-surfaceless /
# llvmpipe (GL 4.5 core, no OSMesa): create a surfaceless EGL desktop-GL 4.5 core context (moderngl is
# unavailable in Alpine, so the Python desktop-GL cell uses PyOpenGL's OpenGL.EGL + OpenGL.GL exactly
# like the sibling render/gles cells) and exercise the GL compute-shader API surface (off-screen
# context / string query / limits / shader compile incl. compile-error / program link incl. link-error
# / SSBO / buffer storage / buffer-base binding / uniform / dispatch / memory-barrier / fence-sync /
# timer query / get-buffer-sub-data / read-map / write-map / copy-sub-data / clear-buffer-data / block
# introspection incl. resource-name + resource-property / error-path validation / boundary +
# oversubscription + large 1M-element dispatch / negative controls) and assert vadd/saxpy/mul operator
# results per-element against numpy. Prints "OPENGL_PY_FULL_API OK <n>" only when every assertion
# passes and the count equals EXPECTED. Runs on-target on every arch (mesa-egl + PyOpenGL) the same way
# as opengl_c_egl. Software rasterizer (llvmpipe), single-threaded, deterministic - no GPU.
#
# The context is a surfaceless EGL desktop-GL context (EGL_OPENGL_BIT / EGL_OPENGL_API), selected via
# PYOPENGL_PLATFORM=egl before OpenGL is imported. Read-back uses ctypes-backed buffers wrapped with
# numpy; the mapped range is copied out through ctypes.from_address. PyOpenGL wraps every GL call with
# an auto glGetError check, so a real GL validation error surfaces as an OpenGL.error.GLError whose
# .err carries the exact GL_INVALID_* enum - error paths assert that enum.
import os
...
# --- surfaceless EGL desktop-GL 4.5 core context (same bring-up idiom as the C EGL cell) ---
...
ok(b"OpenGL" in apis_b, "eglQueryString CLIENT_APIS has OpenGL")
...
ok(bool(EGL.eglChooseConfig(dpy, cfgattr, cfgs, 1, ctypes.byref(ncfg))) and ncfg.value >= 1,
   "eglChooseConfig EGL_OPENGL_BIT")
ok(bool(EGL.eglBindAPI(EGL.EGL_OPENGL_API)), "eglBindAPI EGL_OPENGL_API")
ok(EGL.eglQueryAPI() == EGL.EGL_OPENGL_API, "eglQueryAPI == OPENGL")
ctxattr = (EGL.EGLint * 7)(EGL.EGL_CONTEXT_MAJOR_VERSION, 4, EGL.EGL_CONTEXT_MINOR_VERSION, 5,
