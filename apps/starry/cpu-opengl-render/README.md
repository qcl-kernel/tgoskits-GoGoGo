...
/ mesa-gles / mesa-dri-gallium = llvmpipe) + toolchain + python3/numpy/moderngl via
qemu-user (apk resolves for the target arch), then per cell: cross-compiles the C++ cell with the
staging g++, cross-compiles the Rust cell to dynamic musl, and wires the moderngl Python cell where
py3-moderngl resolves. A capability manifest lists exactly the cells provisioned on
this arch; `run_all.sh` gates on that set (`fail==0 && total==EXPECTED==pass`, `EXPECTED>=1` floor -
the C++ cell is the guaranteed native gate) and never emits a 0-carpet pass.
