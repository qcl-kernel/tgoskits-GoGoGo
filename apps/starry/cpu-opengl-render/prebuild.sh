...
# carries a `.relr.dyn` (SHT_RELR) section older musl-cross binutils ld rejects; zig's bundled LLD reads it.
# Preference order (mirrors the merged sibling carpets):
#   1) ${triple}-g++ (PATH or /opt) - native GNU ABI; used only if its ld can actually link a libEGL .relr.dyn .so.
