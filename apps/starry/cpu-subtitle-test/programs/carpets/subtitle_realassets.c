...
int main(void) {
    ...
    if (abuf) {
        ...
        fprintf(stderr, "  SKIP: badapple.ass absent under %s\n", sub_dir());
    }

    if (!have_any) {
        /* The prebuild always stages both plain-blob assets from the media submodule, so on target their
         * absence is a real provisioning breakage, not a legitimate skip - fail the cell loudly. */
        fprintf(stderr, "  FAIL: no real subtitle assets under %s - submodule/asset staging is broken\n", sub_dir());
        gate_check(&g, 0, "real assets present (staged by prebuild)");
    }
