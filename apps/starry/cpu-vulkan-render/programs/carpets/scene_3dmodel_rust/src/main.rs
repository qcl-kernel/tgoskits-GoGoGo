...
fn mul(a: &[f32; 16], b: &[f32; 16]) -> [f32; 16] {
    let mut r = [0.0f32; 16];
    for cc in 0..4 { for row in 0..4 {
        let mut s = 0.0;
        for k in 0..4 { s += a[k * 4 + row] * b[cc * 4 + k]; }
        r[cc * 4 + row] = s;
    }}
    r
}
fn mv4(a: &[f32; 16], v: &[f32; 4]) -> [f32; 4] {
    let mut o = [0.0f32; 4];
    for row in 0..4 { let mut s = 0.0; for k in 0..4 { s += a[k * 4 + row] * v[k]; } o[row] = s; }
    o
}
...
fn rot_y(a: f32) -> [f32; 16] {
    ...
    let mut r = [0.0f32; 16];
    r[0] = c; r[2] = -sn; r[2 * 4] = sn; r[2 * 4 + 2] = c; r[1 * 4 + 1] = 1.0; r[3 * 4 + 3] = 1.0;
    r
}
fn rot_x(a: f32) -> [f32; 16] {
    let (c, sn) = (a.cos(), a.sin());
    let mut r = [0.0f32; 16];
    r[1 * 4 + 1] = c; r[1 * 4 + 2] = sn; r[2 * 4 + 1] = -sn; r[2 * 4 + 2] = c; r[0] = 1.0; r[3 * 4 + 3] = 1.0;
    r
}
...
impl Ctx {
    ...
    fn mkbuf(&self, data: &[u8], usage: vk::BufferUsageFlags) -> (vk::Buffer, vk::DeviceMemory) {
        ...
        let b = unsafe { self.dev.create_buffer(&vk::BufferCreateInfo::default().size(sz).usage(usage), None) }.expect("buffer");
        let mr = unsafe { self.dev.get_buffer_memory_requirements(b) };
        let ai = vk::MemoryAllocateInfo::default().allocation_size(mr.size).memory_type_index(self.memtype(mr.memory_type_bits, vk::MemoryPropertyFlags::HOST_VISIBLE | vk::MemoryPropertyFlags::HOST_COHERENT));
...
fn main() {
    ...
    let mut c = Ctx {
        ...
        pool: vk::CommandPool::null(), cimg: vk::Image::null(), rbuf: vk::Buffer::null(), rmap: std::ptr::null_mut(),
        buf: vec![0u8; W * H * 4],
    };
    ...
    // depth image
    let dii = vk::ImageCreateInfo::default().image_type(vk::ImageType::TYPE_2D).format(vk::Format::D32_SFLOAT)
        .extent(vk::Extent3D { width: W as u32, height: H as u32, depth: 1 }).mip_levels(1).array_layers(1)
        .samples(vk::SampleCountFlags::TYPE_1).tiling(vk::ImageTiling::OPTIMAL)
    ...
    let dmem = unsafe { c.dev.allocate_memory(&vk::MemoryAllocateInfo::default().allocation_size(dmr.size).memory_type_index(c.memtype(dmr.memory_type_bits, vk::MemoryPropertyFlags::DEVICE_LOCAL)), None) }.unwrap();
    unsafe { c.dev.bind_image_memory(dimg, dmem, 0) }.unwrap();
    let dview = unsafe { c.dev.create_image_view(&vk::ImageViewCreateInfo::default().image(dimg).view_type(vk::ImageViewType::TYPE_2D).format(vk::Format::D32_SFLOAT)
        .subresource_range(vk::ImageSubresourceRange { aspect_mask: vk::ImageAspectFlags::DEPTH, base_mip_level: 0, level_count: 1, base_array_layer: 0, layer_count: 1 }), None) }.expect("depth view");
    s.ok(dview != vk::ImageView::null(), "vkCreateImageView depth");

    // render pass: color + depth
    let att = [
        vk::AttachmentDescription::default().format(vk::Format::R8G8B8A8_UNORM).samples(vk::SampleCountFlags::TYPE_1)
        ...
        vk::AttachmentDescription::default().format(vk::Format::D32_SFLOAT).samples(vk::SampleCountFlags::TYPE_1)
    ...
    let dref = vk::AttachmentReference { attachment: 1, layout: vk::ImageLayout::DEPTH_STENCIL_ATTACHMENT_OPTIMAL };
    let sp = [vk::SubpassDescription::default().pipeline_bind_point(vk::PipelineBindPoint::GRAPHICS).color_attachments(&cref).depth_stencil_attachment(&dref)];
    let rp = unsafe { c.dev.create_render_pass(&vk::RenderPassCreateInfo::default().attachments(&att).subpasses(&sp), None) }.expect("render pass");
    s.ok(rp != vk::RenderPass::null(), "vkCreateRenderPass");
    let fbv = [cview, dview];
    let fb = unsafe { c.dev.create_framebuffer(&vk::FramebufferCreateInfo::default().render_pass(rp).attachments(&fbv).width(W as u32).height(H as u32).layers(1), None) }.expect("framebuffer");
    ...
    s.ok(true, "offscreen R8G8B8A8 + D32_SFLOAT target + readback buffer ready");
    ...
    for i in 0..8 { for k in 0..3 { vc[i][k] = (vpos[i][k] + 1.0) * 0.5; } }
    let idx: [u16; 36] = [
    ...
    for i in 0..8 {
        verts[i * 6] = vpos[i][0]; verts[i * 6 + 1] = vpos[i][1]; verts[i * 6 + 2] = vpos[i][2];
    ...
    let pcr = [vk::PushConstantRange { stage_flags: vk::ShaderStageFlags::VERTEX, offset: 0, size: 64 }];
    let pl = unsafe { c.dev.create_pipeline_layout(&vk::PipelineLayoutCreateInfo::default().push_constant_ranges(&pcr), None) }.unwrap();
    let name = c"main";
    let st = [
    ...
    let vin = vk::PipelineVertexInputStateCreateInfo::default().vertex_binding_descriptions(&bind).vertex_attribute_descriptions(&attr);
    ...
    let scr = [vk::Rect2D { offset: vk::Offset2D { x: 0, y: 0 }, extent: vk::Extent2D { width: W as u32, height: H as u32 } }];
    let vps = vk::PipelineViewportStateCreateInfo::default().viewports(&vp).scissors(&scr);
    ...
    let dss = vk::PipelineDepthStencilStateCreateInfo::default().depth_test_enable(true).depth_write_enable(true).depth_compare_op(vk::CompareOp::LESS).min_depth_bounds(0.0).max_depth_bounds(1.0);
    ...
    let gp = [vk::GraphicsPipelineCreateInfo::default().stages(&st).vertex_input_state(&vin).input_assembly_state(&ia).viewport_state(&vps)
        .rasterization_state(&rs).multisample_state(&ms).depth_stencil_state(&dss).color_blend_state(&cb).layout(pl).render_pass(rp).subpass(0)];
    let pipe = unsafe { c.dev.create_graphics_pipelines(vk::PipelineCache::null(), &gp, None) }.expect("cube pipeline")[0];
    ...
    // draw
    {
        ...
        let rpb = vk::RenderPassBeginInfo::default().render_pass(rp).framebuffer(fb)
        ...
        unsafe {
            ...
            c.dev.cmd_push_constants(cmd, pl, vk::ShaderStageFlags::VERTEX, 0, &mvp_bytes);
            c.dev.cmd_bind_vertex_buffers(cmd, 0, &[vbo], &[0]);
            c.dev.cmd_bind_index_buffer(cmd, ibo, 0, vk::IndexType::UINT16);
    ...
    for i in 0..8 {
        ...
        sx[i] = (ndcx * 0.5 + 0.5) * W as f32; sy[i] = (ndcy * 0.5 + 0.5) * H as f32; sz[i] = ndcz;
    }
    s.ok(sw[0] > 0.0, "reference: all clip.w positive (mesh in front of camera)");
    for t in 0..12 {
        ...
        if maxx > W as i32 { maxx = W as i32; } if maxy > H as i32 { maxy = H as i32; }
        for y in miny..maxy { for x in minx..maxx {
            let pxs = x as f32 + 0.5; let pys = y as f32 + 0.5;
            ...
            if !inside { continue; }
            if w0 < 0.0 || w1 < 0.0 || w2 < 0.0 { w0 = -w0; w1 = -w1; w2 = -w2; }
            let z = w0 * sz[a] + w1 * sz[b] + w2 * sz[cc];
            let idxp = (y as usize) * W + x as usize;
            if z < refz[idxp] {
                refz[idxp] = z; refcov[idxp] = 1;
    ...
    let (mut total, mut mtch, mut covmatch, mut covtotal, mut interior_bad) = (0i32, 0i32, 0i32, 0i32, 0i32);
    for y in 0..H { for x in 0..W {
        ...
        if rcov {
            ...
            let interior = x > 0 && y > 0 && x < W - 1 && y < H - 1
                && refcov[(y - 1) * W + x] != 0 && refcov[(y + 1) * W + x] != 0
                && refcov[y * W + x - 1] != 0 && refcov[y * W + x + 1] != 0;
            if c.peq(x, y, er, eg, eb, 255, 6) { mtch += 1; }
            else if interior { interior_bad += 1; }
        }
    }}
    s.ok(covtotal > 200, "reference: cube covers a substantial area");
    s.ok(covmatch >= (0.97 * total as f32) as i32, "coverage mask matches GPU (>=97% of pixels agree covered/empty)");
    s.ok(interior_bad == 0, "every interior pixel matches perspective-correct Gouraud reference (tol 6)");
    s.ok(mtch >= (0.92 * covtotal as f32) as i32, "92%+ of covered pixels match reference color (edges excluded)");

    {
        ...
        if vx >= 1 && vx < W as i32 - 1 && vy >= 1 && vy < H as i32 - 1 {
            ...
            for dy in -1..=1 { for dx in -1..=1 {
                ...
                if c.px(xx, yy, 0) > 180 && c.px(xx, yy, 1) > 180 && c.px(xx, yy, 2) > 180 { bright = true; }
            }}
            s.ok(bright, "vertex (1,1,1) region is bright (Gouraud white corner)");
        } else {
            s.ok(false, "vertex (1,1,1) projected off-screen (camera mis-set)");
        }
    }
    s.ok(c.peq(0, 0, 0, 0, 0, 255, 1) || refcov[0] == 0, "corner (0,0) background consistent");

    {
        ...
        if refcov[cyp * W + cxp] != 0 {
            ...
            let eg = (refc[idxp][1] * 255.0).round() as i32;
            let eb = (refc[idxp][2] * 255.0).round() as i32;
            s.ok(c.peq(cxp, cyp, er, eg, eb, 255, 8), "center pixel = nearest-face (depth-buffered occlusion) reference color");
        } else {
            s.ok(false, "center pixel not covered (mesh mis-projected)");
        }
    }
    s.ok(!(c.px(1, 1, 0) == c.px(W / 2, H / 2, 0) && c.px(1, 1, 1) == c.px(W / 2, H / 2, 1) && c.px(1, 1, 2) == c.px(W / 2, H / 2, 2)),
