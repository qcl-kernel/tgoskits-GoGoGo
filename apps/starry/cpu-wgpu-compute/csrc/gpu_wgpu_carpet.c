...
int main(void) {
    ...
    /* wgpu-native packs its version into a u32 as major<<24 | minor<<16 |
     * patch<<8 | build, parsed from the WGPU_NATIVE_VERSION string baked in at
     * build time (src/logging.rs wgpuGetVersion). prebuild.sh builds the cdylib
     * from tag WGPU_NATIVE_TAG=v27.0.4.0 with WGPU_NATIVE_VERSION set to that
     * tag (the 27.x series whose webgpu.h - WGPUStringView labels, WGPUFuture
     * callbacks - this carpet is written against), so the high byte reads back
     * as major 27 - a concrete property of the linked runtime, not a
     * compiled-in constant. */
    ok(((wgpuGetVersion() >> 24) & 0xff) == 27, "wgpuGetVersion major == 27 (linked wgpu-native v27.0.4.0 ABI)");
