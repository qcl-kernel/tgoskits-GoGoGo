...
#   - create_compute_pipeline_async (async def in wgpu-py 0.26) driven to completion as a coroutine.
...
def run():
    ...
    # === async compute pipeline (coroutine driven to completion) =============================
    # wgpu-py 0.26 makes create_compute_pipeline_async an `async def`. wgpu-native does not implement
    # async pipeline creation (_CREATE_PIPELINE_ASYNC_IS_IMPLEMENTED is False), so the coroutine takes
    # the synchronous fallback and returns without ever suspending; drive it with a single send(None)
    # (no event loop needed) and take the pipeline from StopIteration.value.
    coro = device.create_compute_pipeline_async(
        layout=pll2, compute={"module": mul_mod, "entry_point": "main"})
    async_pipe = None
    try:
        coro.send(None)
    except StopIteration as done:
        async_pipe = done.value
    ok(async_pipe is not None, "create_compute_pipeline_async coroutine resolves to a pipeline")
