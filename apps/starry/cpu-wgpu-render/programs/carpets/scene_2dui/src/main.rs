...
async fn run() -> i32 {
    ...
    ok(info.backend != wgpu::Backend::Empty, "request_adapter yields a usable adapter");
    ok(
    ...
    let solid_bg = device.create_bind_group(&wgpu::BindGroupDescriptor {
        label: Some("solid-bg"),
        layout: &solid_bgl,
        entries: &[wgpu::BindGroupEntry { binding: 0, resource: solid_ubo.as_entire_binding() }],
    });
    ...
    let bg_a = device.create_bind_group(&wgpu::BindGroupDescriptor {
        label: Some("bgA"),
        layout: &solid_bgl,
        entries: &[wgpu::BindGroupEntry { binding: 0, resource: ubo_a.as_entire_binding() }],
    });
    let bg_b = device.create_bind_group(&wgpu::BindGroupDescriptor {
        label: Some("bgB"),
        layout: &solid_bgl,
        entries: &[wgpu::BindGroupEntry { binding: 0, resource: ubo_b.as_entire_binding() }],
    });
    ...
    let fb = frame(
        [0.0, 0.0, 0.0, 1.0],
        &[
            DrawOp { pipe: &pipe_solid, bind: &bg_a, vbo: &vr1, verts: 6, scissor: None },
            DrawOp { pipe: &pipe_solid, bind: &bg_b, vbo: &vr2, verts: 6, scissor: None },
        ],
    );
    {
        let mut bad = 0;
    ...
    // ---- Scene B: analytic rounded-rect ----
    {
        let rr_ubo = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
            ...
            contents: bytemuck::cast_slice(&[
                12.0f32, 12.0, 52.0, 52.0, 1.0, 1.0, 0.0, 1.0, 8.0, 0.0, 0.0, 0.0, W as f32, H as f32, 0.0, 0.0,
            ]),
            usage: wgpu::BufferUsages::UNIFORM,
        });
        ...
        let rr_bg = device.create_bind_group(&wgpu::BindGroupDescriptor {
            label: Some("rr-bg"),
            layout: &rr_bgl,
            entries: &[wgpu::BindGroupEntry { binding: 0, resource: rr_ubo.as_entire_binding() }],
        });
        ...
        let fb = frame(
            [0.0, 0.0, 0.0, 1.0],
            &[DrawOp { pipe: &pipe_rr, bind: &rr_bg, vbo: &fq, verts: 6, scissor: None }],
        );
    ...
    // ---- Scene C: nine-patch-style scaled border frame ----
    {
        ...
        let bg_blue = device.create_bind_group(&wgpu::BindGroupDescriptor {
            label: Some("bg-blue"),
            layout: &solid_bgl,
            entries: &[wgpu::BindGroupEntry { binding: 0, resource: ubo_blue.as_entire_binding() }],
        });
        let bg_dark = device.create_bind_group(&wgpu::BindGroupDescriptor {
            label: Some("bg-dark"),
            layout: &solid_bgl,
            entries: &[wgpu::BindGroupEntry { binding: 0, resource: ubo_dark.as_entire_binding() }],
        });
        let fb = frame(
            [0.0, 0.0, 0.0, 1.0],
            &[
                DrawOp { pipe: &pipe_solid, bind: &bg_blue, vbo: &vbox, verts: 6, scissor: None },
                DrawOp { pipe: &pipe_solid, bind: &bg_dark, vbo: &vinner, verts: 6, scissor: None },
            ],
        );
        let mut bad = 0;
        for y in 0..H as i32 {
        ...
        ok(
            fb.peq(32, 32, q8(0.1) as i32, q8(0.1) as i32, q8(0.1) as i32, 255, 1),
    ...
    const GLYPH_H: [u8; 8] = [0x00, 0x42, 0x42, 0x7E, 0x42, 0x42, 0x42, 0x00];
    {
        ...
        queue.write_texture(
            ...
            &rgba,
            wgpu::ImageDataLayout { offset: 0, bytes_per_row: Some(32), rows_per_image: Some(8) },
        ...
        let tex_bg = device.create_bind_group(&wgpu::BindGroupDescriptor {
            ...
            layout: &tex_bgl,
            entries: &[
                wgpu::BindGroupEntry { binding: 0, resource: vp_ubo.as_entire_binding() },
                wgpu::BindGroupEntry { binding: 1, resource: wgpu::BindingResource::TextureView(&gview) },
        ...
        let gq: [V2UV; 6] = [
            V2UV { pos: [20.0, 20.0], uv: [0.0, 0.0] },
            V2UV { pos: [28.0, 20.0], uv: [1.0, 0.0] },
            V2UV { pos: [20.0, 28.0], uv: [0.0, 1.0] },
            V2UV { pos: [20.0, 28.0], uv: [0.0, 1.0] },
            V2UV { pos: [28.0, 20.0], uv: [1.0, 0.0] },
            V2UV { pos: [28.0, 28.0], uv: [1.0, 1.0] },
        ];
        ...
        let fb = frame(
            [0.0, 0.0, 0.0, 1.0],
            &[DrawOp { pipe: &pipe_tex, bind: &tex_bg, vbo: &gvbo, verts: 6, scissor: None }],
        );
        let mut bad = 0;
        for dy in 0..8i32 {
            for dx in 0..8i32 {
    ...
    // ---- Scene E: scissor-clipped fill ----
    {
        ...
        let fb = frame(
            [0.0, 0.0, 0.0, 1.0],
            &[DrawOp { pipe: &pipe_solid, bind: &solid_bg, vbo: &fq, verts: 6, scissor: Some((16, 16, 20, 20)) }],
        );
        let mut bad = 0;
        for y in 0..H as i32 {
            for x in 0..W as i32 {
    ...
    // ---- Scene F: MULTI-LAYER Porter-Duff over compositing ----
    {
        ...
        let layers = [
            L { r: 1.0, g: 0.0, b: 0.0, a: 0.50, x0: 8.0, y0: 8.0, x1: 56.0, y1: 56.0 },
            L { r: 0.0, g: 1.0, b: 0.0, a: 0.25, x0: 12.0, y0: 12.0, x1: 52.0, y1: 52.0 },
            L { r: 0.0, g: 0.0, b: 1.0, a: 0.75, x0: 16.0, y0: 16.0, x1: 48.0, y1: 48.0 },
        ];
        ...
        let ops: Vec<DrawOp> = (0..3)
            .map(|i| DrawOp { pipe: &pipe_blend, bind: &ops_bgs[i], vbo: &ops_vbos[i], verts: 6, scissor: None })
            .collect();
        let fb = frame([bg[0] as f64, bg[1] as f64, bg[2] as f64, bg[3] as f64], &ops);
        ...
        for y in 0..H as i32 {
            for x in 0..W as i32 {
                let e = composite(x, y);
                if !fb.peq(x, y, q8(e[0]) as i32, q8(e[1]) as i32, q8(e[2]) as i32, q8(e[3]) as i32, 2) {
                    bad += 1;
                }
            }
        }
        ...
        {
            let mut c = bg;
            let ls = [[1.0, 0.0, 0.0, 0.5f32], [0.0, 1.0, 0.0, 0.25], [0.0, 0.0, 1.0, 0.75]];
            for li in ls {
                let a_s = li[3];
                for k in 0..4 {
            ...
            ok(
                fb.peq(32, 32, q8(c[0]) as i32, q8(c[1]) as i32, q8(c[2]) as i32, q8(c[3]) as i32, 2),
        ...
        }
        {
            ...
            ok(
                fb.peq(10, 32, q8(er) as i32, q8(eg) as i32, q8(eb) as i32, q8(ea) as i32, 2),
