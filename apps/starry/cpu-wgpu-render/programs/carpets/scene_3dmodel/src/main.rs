...
async fn run() -> i32 {
    ...
    let mut verts = [Vtx { pos: [0.0; 3], col: [0.0; 3] }; 8];
    for i in 0..8 {
        verts[i] = Vtx { pos: VP[i], col: vc[i] };
    }
    ...
    let (mut total, mut r#match, mut covmatch, mut covtotal, mut interior_bad) = (0i32, 0i32, 0i32, 0i32, 0i32);
    for y in 0..H {
        for x in 0..W {
            total += 1;
