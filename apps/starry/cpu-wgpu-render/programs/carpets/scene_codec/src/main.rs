...
async fn run() -> i32 {
    ...
    let fsq: [V2UV; 4] = [
        V2UV { pos: [-1.0, 1.0], uv: [0.0, 0.0] },
        V2UV { pos: [1.0, 1.0], uv: [1.0, 0.0] },
        V2UV { pos: [-1.0, -1.0], uv: [0.0, 1.0] },
        V2UV { pos: [1.0, -1.0], uv: [1.0, 1.0] },
    ];
    ...
    // ============ (1) YUV -> RGB, BT.601 full-range ============
    {
        ...
        let bind = device.create_bind_group(&wgpu::BindGroupDescriptor {
            ...
            layout: &bgl,
            entries: &[
                wgpu::BindGroupEntry { binding: 0, resource: wgpu::BindingResource::TextureView(&vy) },
                wgpu::BindGroupEntry { binding: 1, resource: wgpu::BindingResource::TextureView(&vu) },
                wgpu::BindGroupEntry { binding: 2, resource: wgpu::BindingResource::TextureView(&vv) },
        ...
        ok(checked == (pw * ph) as i32, "YUV->RGB checked all 32x32 output pixels");
    ...
    // ============ (2) chroma 4:2:0 -> 4:4:4 NEAREST upsample ============
    {
        ...
        for yy in 0..oh as i32 {
            for xx in 0..ow as i32 {
                ...
                if !fb.peq(xx, yy, src[i] as i32, src[i + 1] as i32, src[i + 2] as i32, 255, 1) {
                    bad += 1;
                }
            }
        }
        ok(bad == 0, "4:2:0->4:4:4 NEAREST upsample: each output texel = replicated source block (closed form)");
        ok(fb.peq(0, 0, src[0] as i32, src[1] as i32, src[2] as i32, 255, 1), "upsample (0,0) = src(0,0)");
        let i33 = (3 * sw + 3) * 4;
        ok(
            fb.peq(15, 15, src[i33] as i32, src[i33 + 1] as i32, src[i33 + 2] as i32, 255, 1),
    ...
    // ============ (4) codec round-trip identities (CPU path) ============
    {
        ...
        for n in 0..N {
            let mut s = xc[0];
            for k in 1..N {
                s += 2.0 * xc[k] * (std::f64::consts::PI / N as f64 * (n as f64 + 0.5) * k as f64).cos();
            }
            yv[n] = s / N as f64;
        }
    ...
    // ---- Negative control ----
    {
        ...
        let mut enc = device.create_command_encoder(&wgpu::CommandEncoderDescriptor { label: Some("neg") });
        {
