...
impl Fb {
    ...
    fn peq(&self, x: u32, y: u32, r: u8, g: u8, b: u8, a: u8, tol: i32) -> bool {
        ...
        d(self.p(x, y, 0), r) && d(self.p(x, y, 1), g) && d(self.p(x, y, 2), b) && d(self.p(x, y, 3), a)
    }
...
async fn run() -> i32 {
    ...
    // --- Blend factor + op matrix -----------------------------------------------------------------
    {
        ...
        let fb = frame(
            [0.5, 0.5, 0.5, 1.0],
            Draw { pipe: Some(&p), bind: Some(&ubo_bg), vbo: Some(&vbo), verts: 4, scissor: None, viewport: None },
        );
    ...
    // --- Depth-compare matrix (all 8 CompareFunction; z=0.5 quad vs clear-depth 0.75) -------------
    {
        ...
        let cases = [
            ...
            (wgpu::CompareFunction::GreaterEqual, false, "depth GreaterEqual"),
        ...
        for (cmp, draws, name) in cases {
            ...
            ok(
                fb.peq(W / 2, H / 2, 0, 255, 0, 255, 2) == draws,
                name,
            );
        }
    }
    ...
    set_color(1.0, 0.0, 0.0, 1.0);
    {
        ...
        let fb = frame(
            [0.0, 0.0, 0.0, 1.0],
            Draw { pipe: Some(&p_ccw), bind: Some(&ubo_bg), vbo: Some(&vbo), verts: 4, scissor: None, viewport: None },
        );
        ...
        let fb = frame(
            [0.0, 0.0, 0.0, 1.0],
            Draw { pipe: Some(&p_cw), bind: Some(&ubo_bg), vbo: Some(&vbo), verts: 4, scissor: None, viewport: None },
        );
        ...
        let fb = frame(
            [0.0, 0.0, 0.0, 1.0],
            Draw { pipe: Some(&p_front), bind: Some(&ubo_bg), vbo: Some(&vbo), verts: 4, scissor: None, viewport: None },
        );
    ...
    set_color(1.0, 1.0, 1.0, 1.0);
    {
        ...
        let fb = frame(
            [0.0, 0.0, 0.0, 1.0],
            Draw { pipe: Some(&p_r), bind: Some(&ubo_bg), vbo: Some(&vbo), verts: 4, scissor: None, viewport: None },
        );
        ok(
        ...
        let fb = frame(
            [0.0, 0.0, 0.0, 1.0],
            Draw { pipe: Some(&p_all), bind: Some(&ubo_bg), vbo: Some(&vbo), verts: 4, scissor: None, viewport: None },
        );
        ok(
    ...
    // --- Format feature + limit queries -----------------------------------------------------------
    {
        ...
        ok(lim.max_color_attachments >= 1, "limits.max_color_attachments >= 1");
    }
    ...
    ok(!fb.all_eq(0, 255, 0, 255, 2), "negative control: red buffer is NOT green");
