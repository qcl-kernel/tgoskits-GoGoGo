// SPDX-License-Identifier: Apache-2.0
//! Deterministic owner-only SMP push/pull scheduler contracts.

use ax_task::{
    CpuId, CpuSet, DEFAULT_BALANCE_INTERVAL_NS, FairMode, Nice, RtPriority, SchedulePolicy,
    TaskSystem, TaskSystemConfig, ThreadSpec, ThreadState, WakeResult,
};

pub mod support;
use support::TaskSystemClockTestExt;

#[test]
fn idle_cpu_requests_source_owned_rt_handoff() {
    let (system, mut cpu0, mut cpu1, idle1) = online_pair();
    let high = ready_thread(&system, SchedulePolicy::fifo(RtPriority::new(90).unwrap()));
    let low = ready_thread(&system, SchedulePolicy::fifo(RtPriority::new(80).unwrap()));
    system.enqueue_at(cpu0.as_mut(), high.id(), 0).unwrap();
    system.enqueue_at(cpu0.as_mut(), low.id(), 0).unwrap();
    assert_eq!(
        system.schedule_at(cpu1.as_mut(), 0).unwrap().next(),
        idle1.id()
    );

    assert!(system.request_idle_pull(cpu1.as_mut()).unwrap());
    system.drain_owner_control_at(cpu0.as_mut(), 1).unwrap();
    system.drain_owner_control_at(cpu1.as_mut(), 1).unwrap();

    assert_eq!(
        system.schedule_at(cpu1.as_mut(), 1).unwrap().next(),
        low.id()
    );
    assert_eq!(
        system.schedule_at(cpu0.as_mut(), 1).unwrap().next(),
        high.id()
    );
}

#[test]
fn coalesced_idle_requests_leave_final_selection_to_the_source_owner() {
    let (system, mut cpu0, mut cpu1, idle1) = online_pair();
    let high = ready_thread(&system, SchedulePolicy::fifo(RtPriority::new(90).unwrap()));
    let low = ready_thread(&system, SchedulePolicy::fifo(RtPriority::new(80).unwrap()));
    system.enqueue_at(cpu0.as_mut(), high.id(), 0).unwrap();
    system.enqueue_at(cpu0.as_mut(), low.id(), 0).unwrap();
    assert_eq!(
        system.schedule_at(cpu1.as_mut(), 0).unwrap().next(),
        idle1.id()
    );

    assert!(system.request_idle_pull(cpu1.as_mut()).unwrap());
    assert!(system.request_idle_pull(cpu1.as_mut()).unwrap());
    assert_eq!(
        support::ipi_count(0),
        1,
        "generic scheduler work must not suppress the first physical owner-delivery edge"
    );
    system
        .set_affinity(low.id(), singleton_affinity(2, 0))
        .unwrap();

    let drained = system.drain_owner_control_at(cpu0.as_mut(), 1).unwrap();
    assert!(drained.drained() <= ax_task::DEFAULT_BATCH_LIMIT);
    system.drain_owner_control_at(cpu1.as_mut(), 1).unwrap();
    assert_eq!(cpu1.queued_summary(), 0);
}

#[test]
fn stale_idle_pull_request_cannot_overfill_a_target_that_became_runnable() {
    let (system, mut cpu0, mut cpu1, idle1) = online_pair();
    let high = ready_thread(&system, SchedulePolicy::fifo(RtPriority::new(90).unwrap()));
    let low = ready_thread(&system, SchedulePolicy::fifo(RtPriority::new(80).unwrap()));
    system.enqueue_at(cpu0.as_mut(), high.id(), 0).unwrap();
    system.enqueue_at(cpu0.as_mut(), low.id(), 0).unwrap();
    assert_eq!(
        system.schedule_at(cpu1.as_mut(), 0).unwrap().next(),
        idle1.id()
    );

    assert!(system.request_idle_pull(cpu1.as_mut()).unwrap());
    let local = ready_thread(&system, SchedulePolicy::default());
    system.enqueue_at(cpu1.as_mut(), local.id(), 1).unwrap();
    assert_eq!(cpu1.queued_summary(), 1);

    system.drain_owner_control_at(cpu0.as_mut(), 1).unwrap();
    system.drain_owner_control_at(cpu1.as_mut(), 1).unwrap();

    assert_eq!(
        cpu0.queued_summary(),
        2,
        "the source must retain its candidate when the target cancels an uncommitted pull"
    );
    assert_eq!(
        cpu1.queued_summary(),
        1,
        "work arriving before the pull commits must invalidate the stale idle request"
    );
}

#[test]
fn scheduling_overloaded_rt_queue_pushes_one_candidate_automatically() {
    let (system, mut cpu0, mut cpu1, _idle1) = online_pair();
    let high = ready_thread(&system, SchedulePolicy::fifo(RtPriority::new(90).unwrap()));
    let middle = ready_thread(&system, SchedulePolicy::fifo(RtPriority::new(80).unwrap()));
    let low = ready_thread(&system, SchedulePolicy::fifo(RtPriority::new(70).unwrap()));
    for thread in [&high, &middle, &low] {
        system.enqueue_at(cpu0.as_mut(), thread.id(), 0).unwrap();
    }

    assert_eq!(
        system.schedule_at(cpu0.as_mut(), 0).unwrap().next(),
        high.id()
    );
    system.drain_owner_control_at(cpu1.as_mut(), 1).unwrap();

    assert_eq!(cpu0.queued_summary(), 1);
    assert_eq!(cpu1.queued_summary(), 1);
    assert_eq!(
        system.schedule_at(cpu1.as_mut(), 1).unwrap().next(),
        middle.id()
    );
}

#[test]
fn load_summary_contains_only_fair_domain_load() {
    let (system, mut cpu0, _cpu1, _idle1) = online_pair();
    let current = ready_thread(&system, SchedulePolicy::fifo(RtPriority::new(90).unwrap()));
    let queued = ready_thread(&system, SchedulePolicy::fifo(RtPriority::new(80).unwrap()));
    system.enqueue_at(cpu0.as_mut(), current.id(), 0).unwrap();
    assert_eq!(
        system.schedule_at(cpu0.as_mut(), 0).unwrap().next(),
        current.id()
    );
    let before = cpu0.load_summary().epoch();
    system.enqueue_at(cpu0.as_mut(), queued.id(), 1).unwrap();

    let summary = cpu0.load_summary();
    assert!(summary.epoch() > before);
    assert_eq!(summary.queued_count(), 1);
    assert_eq!(summary.nr_running(), 2);
    assert_eq!(summary.fair_demand(), 0);
    assert_eq!(summary.workload_demand(), 2_048);
    assert!(!summary.has_pushable_fair());
}

#[test]
fn initial_fair_placement_uses_an_idle_allowed_cpu() {
    let (system, mut cpu0, mut cpu1, idle1) = online_pair();
    let running = ready_thread(&system, SchedulePolicy::default());
    system.enqueue_at(cpu0.as_mut(), running.id(), 0).unwrap();
    assert_eq!(
        system.schedule_at(cpu0.as_mut(), 0).unwrap().next(),
        running.id()
    );
    assert_eq!(
        system.schedule_at(cpu1.as_mut(), 0).unwrap().next(),
        idle1.id()
    );
    assert_eq!(cpu0.queued_summary(), 0);
    assert_eq!(cpu1.queued_summary(), 0);
    assert_eq!(cpu0.load_summary().nr_running(), 1);
    assert_eq!(cpu1.load_summary().nr_running(), 0);

    let placed = ready_thread(&system, SchedulePolicy::default());
    system
        .place_ready_at(cpu0.as_mut(), placed.id(), 1)
        .unwrap();

    system.drain_owner_control_at(cpu1.as_mut(), 1).unwrap();
    assert_eq!(
        system.schedule_at(cpu1.as_mut(), 1).unwrap().next(),
        placed.id()
    );
}

#[test]
fn initial_fair_placement_accounts_for_in_flight_migrations() {
    let (system, mut cpu0, mut cpu1, idle1) = online_pair();
    let running = ready_thread(&system, SchedulePolicy::default());
    system.enqueue_at(cpu0.as_mut(), running.id(), 0).unwrap();
    assert_eq!(
        system.schedule_at(cpu0.as_mut(), 0).unwrap().next(),
        running.id()
    );
    assert_eq!(
        system.schedule_at(cpu1.as_mut(), 0).unwrap().next(),
        idle1.id()
    );

    let placed = (0..4)
        .map(|_| {
            let thread = ready_thread(&system, SchedulePolicy::default());
            system
                .place_ready_at(cpu0.as_mut(), thread.id(), 1)
                .unwrap();
            thread
        })
        .collect::<Vec<_>>();

    assert_eq!(
        cpu0.queued_summary(),
        2,
        "unconsumed remote placements must participate in subsequent CPU selection"
    );
    system.drain_owner_control_at(cpu1.as_mut(), 1).unwrap();
    assert_eq!(cpu1.queued_summary(), 2);
    drop(placed);
}

#[test]
fn in_flight_migration_reserves_the_exact_fair_demand() {
    let (system, mut cpu0, mut cpu1, _idle1) = online_pair();
    for _ in 0..2 {
        let local = ready_thread(&system, SchedulePolicy::default());
        system.enqueue_at(cpu0.as_mut(), local.id(), 0).unwrap();
    }
    let heavy = ready_thread(
        &system,
        SchedulePolicy::fair(Nice::new(-20).unwrap(), FairMode::Normal),
    );
    system
        .set_affinity(heavy.id(), singleton_affinity(2, 1))
        .unwrap();
    system.place_ready_at(cpu0.as_mut(), heavy.id(), 1).unwrap();

    let local = ready_thread(&system, SchedulePolicy::default());
    system.place_ready_at(cpu0.as_mut(), local.id(), 1).unwrap();

    assert_eq!(
        cpu0.queued_summary(),
        3,
        "the pending nice -20 carrier must outweigh two local normal tasks during placement"
    );
    system.drain_owner_control_at(cpu1.as_mut(), 1).unwrap();
    assert_eq!(cpu1.queued_summary(), 1);
}

#[test]
fn idle_pull_prefers_rt_work_over_a_larger_fair_queue() {
    let (system, mut cpus, idle2) = online_triple();
    for cpu in &mut cpus {
        let _idle = system.schedule_at(cpu.as_mut(), 0).unwrap();
    }
    for _ in 0..3 {
        let fair = ready_thread(&system, SchedulePolicy::default());
        system.enqueue_at(cpus[0].as_mut(), fair.id(), 0).unwrap();
    }
    let high = ready_thread(&system, SchedulePolicy::fifo(RtPriority::new(90).unwrap()));
    let low = ready_thread(&system, SchedulePolicy::fifo(RtPriority::new(80).unwrap()));
    system.enqueue_at(cpus[1].as_mut(), high.id(), 0).unwrap();
    system.enqueue_at(cpus[1].as_mut(), low.id(), 0).unwrap();

    assert!(system.request_idle_pull(cpus[2].as_mut()).unwrap());
    system.drain_owner_control_at(cpus[0].as_mut(), 1).unwrap();
    system.drain_owner_control_at(cpus[1].as_mut(), 1).unwrap();
    system.drain_owner_control_at(cpus[2].as_mut(), 1).unwrap();

    assert_eq!(
        system.schedule_at(cpus[2].as_mut(), 1).unwrap().next(),
        low.id()
    );
    assert_ne!(low.id(), idle2.id());
}

#[test]
fn idle_pull_uses_weighted_demand_not_task_count_within_fair_class() {
    let (system, mut cpus, _idle2) = online_triple();
    for cpu in &mut cpus {
        let _idle = system.schedule_at(cpu.as_mut(), 0).unwrap();
    }

    let lightly_loaded = (0..2)
        .map(|_| {
            let thread = ready_thread(
                &system,
                SchedulePolicy::fair(Nice::new(-20).unwrap(), FairMode::Normal),
            );
            system.enqueue_at(cpus[0].as_mut(), thread.id(), 0).unwrap();
            thread
        })
        .collect::<Vec<_>>();
    let heavily_loaded = (0..5)
        .map(|_| {
            let thread = ready_thread(
                &system,
                SchedulePolicy::fair(Nice::new(19).unwrap(), FairMode::Normal),
            );
            system.enqueue_at(cpus[1].as_mut(), thread.id(), 0).unwrap();
            thread
        })
        .collect::<Vec<_>>();

    let light = cpus[0].load_summary();
    let heavy = cpus[1].load_summary();
    assert_eq!(light.nr_running(), 2);
    assert_eq!(heavy.nr_running(), 5);
    assert!(
        light.fair_demand() > heavy.fair_demand(),
        "the fixture must expose that nice-weighted fair demand, not task count, selects the \
         source"
    );

    support::set_monotonic_ns(DEFAULT_BALANCE_INTERVAL_NS);
    support::set_scheduler_ns(DEFAULT_BALANCE_INTERVAL_NS);
    assert!(system.request_idle_pull(cpus[2].as_mut()).unwrap());
    for cpu in &mut cpus {
        system
            .drain_owner_control_at(cpu.as_mut(), DEFAULT_BALANCE_INTERVAL_NS)
            .unwrap();
    }
    let pulled = system
        .schedule_at(cpus[2].as_mut(), DEFAULT_BALANCE_INTERVAL_NS)
        .unwrap()
        .next();
    assert!(
        lightly_loaded.iter().any(|thread| thread.id() == pulled),
        "the idle CPU must pull from the source with the greatest nice-weighted demand, not the \
         source with the largest task count"
    );
    assert!(heavily_loaded.iter().all(|thread| thread.id() != pulled));
}

#[test]
fn initial_fair_placement_uses_weighted_demand_not_task_count() {
    let (system, mut cpu0, mut cpu1, _idle1) = online_pair();
    let heavy = ready_thread(
        &system,
        SchedulePolicy::fair(Nice::new(-20).unwrap(), FairMode::Normal),
    );
    system.enqueue_at(cpu0.as_mut(), heavy.id(), 0).unwrap();
    let light_policy = SchedulePolicy::fair(Nice::new(19).unwrap(), FairMode::Normal);
    for _ in 0..2 {
        let light = ready_thread(&system, light_policy);
        system.enqueue_at(cpu1.as_mut(), light.id(), 0).unwrap();
    }

    let placed = ready_thread(&system, SchedulePolicy::default());
    system
        .place_ready_at(cpu0.as_mut(), placed.id(), 1)
        .unwrap();

    system.drain_owner_control_at(cpu1.as_mut(), 1).unwrap();
    assert_eq!(
        system.schedule_at(cpu1.as_mut(), 1).unwrap().next(),
        placed.id(),
        "the new fair task must target the lower weighted-demand CPU even when it owns more tasks"
    );
}

#[test]
fn balance_never_hands_off_a_thread_that_is_still_on_cpu() {
    let (system, mut cpu0, mut cpu1, idle1) = online_pair();
    let previous = ready_thread(&system, SchedulePolicy::default());
    let preemptor = ready_thread(&system, SchedulePolicy::fifo(RtPriority::new(90).unwrap()));
    let pinned = ready_thread(&system, SchedulePolicy::default());
    system
        .set_affinity(pinned.id(), singleton_affinity(2, 0))
        .unwrap();
    system.enqueue_at(cpu0.as_mut(), previous.id(), 0).unwrap();
    assert_eq!(
        system.schedule_at(cpu0.as_mut(), 0).unwrap().next(),
        previous.id()
    );
    system.enqueue_at(cpu0.as_mut(), preemptor.id(), 1).unwrap();
    system.enqueue_at(cpu0.as_mut(), pinned.id(), 1).unwrap();

    assert_eq!(
        system.schedule_at(cpu0.as_mut(), 1).unwrap().next(),
        preemptor.id()
    );
    system.drain_owner_control_at(cpu1.as_mut(), 1).unwrap();
    assert_eq!(cpu1.queued_summary(), 0);
    assert_eq!(
        system.schedule_at(cpu1.as_mut(), 1).unwrap().next(),
        idle1.id()
    );
}

#[test]
fn fair_push_waits_for_the_configured_balance_interval() {
    let (system, mut cpu0, mut cpu1, idle1) = online_pair();
    for _ in 0..3 {
        let fair = ready_thread(&system, SchedulePolicy::default());
        system.enqueue_at(cpu0.as_mut(), fair.id(), 0).unwrap();
    }

    let _first = system.schedule_at(cpu0.as_mut(), 0).unwrap();
    system.drain_owner_control_at(cpu1.as_mut(), 0).unwrap();
    assert_eq!(cpu1.queued_summary(), 0);
    assert_eq!(
        system.schedule_at(cpu1.as_mut(), 0).unwrap().next(),
        idle1.id()
    );

    let _second = system
        .schedule_at(cpu0.as_mut(), DEFAULT_BALANCE_INTERVAL_NS)
        .unwrap();
    system
        .drain_owner_control_at(cpu1.as_mut(), DEFAULT_BALANCE_INTERVAL_NS)
        .unwrap();
    assert_eq!(cpu1.queued_summary(), 1);
}

#[test]
fn fair_balance_deadline_is_relative_to_cpu_online_time() {
    const BOOT_NOW_NS: u64 = 30_000_000_000;

    support::clear_handles();
    support::set_monotonic_ns(BOOT_NOW_NS);
    support::set_scheduler_ns(BOOT_NOW_NS);
    let config = TaskSystemConfig::new(4);
    let fair_slice_ns = config.fair_slice_ns();
    let system = TaskSystem::new(config).unwrap();
    let mut cpus = (0..4)
        .map(|cpu| system.create_cpu_local(CpuId::new(cpu)).unwrap())
        .collect::<Vec<_>>();
    for cpu in &mut cpus {
        system
            .register_idle_thread(
                cpu.as_mut(),
                ThreadSpec::new(SchedulePolicy::fair(Nice::ZERO, FairMode::Idle)),
            )
            .unwrap();
        system.bring_cpu_online(cpu.as_mut()).unwrap();
    }
    for _ in 0..3 {
        let fair = ready_thread(&system, SchedulePolicy::default());
        system
            .enqueue_at(cpus[0].as_mut(), fair.id(), BOOT_NOW_NS)
            .unwrap();
    }

    let _first = system.schedule_at(cpus[0].as_mut(), BOOT_NOW_NS).unwrap();
    assert_eq!(
        support::last_oneshot_ns(),
        BOOT_NOW_NS + fair_slice_ns / 2,
        "an online CPU must not program an already-expired balance duration as an absolute \
         deadline"
    );
    for cpu in cpus.iter_mut().skip(1) {
        system
            .drain_owner_control_at(cpu.as_mut(), BOOT_NOW_NS)
            .unwrap();
        assert_eq!(
            cpu.queued_summary(),
            0,
            "the first runnable batch must receive one full balance interval locally"
        );
    }

    let balance_now = BOOT_NOW_NS + DEFAULT_BALANCE_INTERVAL_NS;
    support::install_handles(
        (&system as *const TaskSystem).expose_provenance(),
        cpus[0].as_mut(),
    );
    support::set_monotonic_ns(balance_now);
    support::set_scheduler_ns(balance_now);
    let event = ax_task::on_clock_event(
        ax_task::runtime::MonotonicInstant::from_nanos(balance_now).unwrap(),
        ax_task::DEFAULT_BATCH_LIMIT,
    )
    .unwrap();
    assert!(
        event.pending(),
        "the physical clockevent must publish periodic balance work before the scheduler consumes \
         it"
    );
    let _second = system.schedule_at(cpus[0].as_mut(), balance_now).unwrap();
    assert_eq!(
        support::last_oneshot_ns(),
        balance_now + fair_slice_ns / 2,
        "the owner must reprogram the timer after advancing the balance deadline"
    );
    for cpu in cpus.iter_mut().skip(1) {
        system
            .drain_owner_control_at(cpu.as_mut(), balance_now)
            .unwrap();
    }
    assert_eq!(
        cpus.iter()
            .skip(1)
            .map(|cpu| cpu.queued_summary())
            .sum::<usize>(),
        1
    );
    support::clear_handles();
}

#[test]
fn direct_wake_is_migrated_after_a_new_affinity_generation() {
    support::clear_handles();
    let system = Box::pin(TaskSystem::new(TaskSystemConfig::new(2)).unwrap());
    let mut cpu0 = system.create_cpu_local(CpuId::new(0)).unwrap();
    let mut cpu1 = system.create_cpu_local(CpuId::new(1)).unwrap();
    let blocked = system
        .install_bootstrap_thread(cpu0.as_mut(), ThreadSpec::new(SchedulePolicy::default()))
        .unwrap();
    for cpu in [&mut cpu0, &mut cpu1] {
        system
            .register_idle_thread(
                cpu.as_mut(),
                ThreadSpec::new(SchedulePolicy::fair(Nice::ZERO, FairMode::Idle)),
            )
            .unwrap();
        system.bring_cpu_online(cpu.as_mut()).unwrap();
    }
    system.block_current_at(cpu0.as_mut(), 0).unwrap();
    system.complete_context_switch(cpu0.as_mut()).unwrap();

    support::install_handles(
        (system.as_ref().get_ref() as *const TaskSystem).expose_provenance(),
        cpu0.as_mut(),
    );
    support::install_cpu(1, cpu1.as_mut());
    support::set_online_cpu_count(2);

    let wake = blocked.wake_handle();
    assert_eq!(blocked.state(), ThreadState::Blocked);
    assert_eq!(blocked.assigned_cpu(), Some(CpuId::new(0)));
    assert_eq!(wake.wake(), WakeResult::Notified);
    assert_eq!(blocked.state(), ThreadState::Ready);
    assert_eq!(blocked.assigned_cpu(), Some(CpuId::new(0)));
    assert_eq!(
        cpu0.queued_summary(),
        1,
        "direct wake must commit queue membership and remotely visible load in one rq transaction"
    );
    system
        .set_affinity(blocked.id(), singleton_affinity(2, 1))
        .unwrap();
    assert_eq!(cpu0.queued_summary(), 1);
    system.drain_owner_control_at(cpu0.as_mut(), 1).unwrap();
    assert_eq!(cpu0.queued_summary(), 0);
    system.drain_owner_control_at(cpu1.as_mut(), 1).unwrap();
    assert_eq!(cpu1.queued_summary(), 1);
    assert_eq!(
        system.schedule_at(cpu1.as_mut(), 1).unwrap().next(),
        blocked.id()
    );
    support::clear_handles();
}

#[test]
fn in_flight_migration_is_forwarded_to_latest_affinity_target() {
    support::clear_handles();
    let system = TaskSystem::new(TaskSystemConfig::new(3)).unwrap();
    let mut cpus = (0..3)
        .map(|cpu| system.create_cpu_local(CpuId::new(cpu)).unwrap())
        .collect::<Vec<_>>();
    for cpu in &mut cpus {
        system
            .register_idle_thread(
                cpu.as_mut(),
                ThreadSpec::new(SchedulePolicy::fair(Nice::ZERO, FairMode::Idle)),
            )
            .unwrap();
        system.bring_cpu_online(cpu.as_mut()).unwrap();
    }
    let thread = ready_thread(&system, SchedulePolicy::default());
    system.enqueue_at(cpus[0].as_mut(), thread.id(), 0).unwrap();

    system
        .set_affinity(thread.id(), singleton_affinity(3, 1))
        .unwrap();
    system.drain_owner_control_at(cpus[0].as_mut(), 1).unwrap();
    system
        .set_affinity(thread.id(), singleton_affinity(3, 2))
        .unwrap();
    system.drain_owner_control_at(cpus[1].as_mut(), 2).unwrap();
    assert_eq!(cpus[1].queued_summary(), 0);
    system.drain_owner_control_at(cpus[2].as_mut(), 3).unwrap();
    assert_eq!(cpus[2].queued_summary(), 1);
    assert_eq!(
        system.schedule_at(cpus[2].as_mut(), 3).unwrap().next(),
        thread.id()
    );
}

#[test]
fn remote_affinity_completion_waits_for_the_destination_owner() {
    support::clear_handles();
    let system = TaskSystem::new(TaskSystemConfig::new(2)).unwrap();
    let mut cpu0 = system.create_cpu_local(CpuId::new(0)).unwrap();
    let mut cpu1 = system.create_cpu_local(CpuId::new(1)).unwrap();
    for cpu in [&mut cpu0, &mut cpu1] {
        system
            .register_idle_thread(
                cpu.as_mut(),
                ThreadSpec::new(SchedulePolicy::fair(Nice::ZERO, FairMode::Idle)),
            )
            .unwrap();
        system.bring_cpu_online(cpu.as_mut()).unwrap();
    }
    let thread = ready_thread(&system, SchedulePolicy::default());
    system.enqueue_at(cpu0.as_mut(), thread.id(), 0).unwrap();

    let request = system
        .request_affinity(thread.id(), singleton_affinity(2, 1))
        .unwrap();
    assert_eq!(
        request.try_result(),
        None,
        "publishing the owner request is not migration completion"
    );

    system.drain_owner_control_at(cpu0.as_mut(), 1).unwrap();
    assert_eq!(
        request.try_result(),
        None,
        "detaching from the source is not destination ownership"
    );
    system.drain_owner_control_at(cpu1.as_mut(), 2).unwrap();
    assert_eq!(request.try_result(), Some(Ok(())));
    support::clear_handles();
}

#[test]
fn concurrent_affinity_waiters_complete_only_after_the_latest_destination() {
    support::clear_handles();
    let system = TaskSystem::new(TaskSystemConfig::new(3)).unwrap();
    let mut cpus = (0..3)
        .map(|cpu| system.create_cpu_local(CpuId::new(cpu)).unwrap())
        .collect::<Vec<_>>();
    for cpu in &mut cpus {
        system
            .register_idle_thread(
                cpu.as_mut(),
                ThreadSpec::new(SchedulePolicy::fair(Nice::ZERO, FairMode::Idle)),
            )
            .unwrap();
        system.bring_cpu_online(cpu.as_mut()).unwrap();
    }
    let thread = ready_thread(&system, SchedulePolicy::default());
    system.enqueue_at(cpus[0].as_mut(), thread.id(), 0).unwrap();

    let first = system
        .request_affinity(thread.id(), singleton_affinity(3, 1))
        .unwrap();
    system.drain_owner_control_at(cpus[0].as_mut(), 1).unwrap();
    let latest = system
        .request_affinity(thread.id(), singleton_affinity(3, 2))
        .unwrap();

    system.drain_owner_control_at(cpus[1].as_mut(), 2).unwrap();
    assert_eq!(first.try_result(), None);
    assert_eq!(latest.try_result(), None);

    system.drain_owner_control_at(cpus[2].as_mut(), 3).unwrap();
    assert_eq!(first.try_result(), Some(Ok(())));
    assert_eq!(latest.try_result(), Some(Ok(())));
    support::clear_handles();
}

#[test]
fn exiting_target_resolves_a_pending_affinity_waiter() {
    let (system, mut cpu0, mut cpu1, _idle1) = online_pair();
    let thread = ready_thread(&system, SchedulePolicy::default());
    system.enqueue_at(cpu0.as_mut(), thread.id(), 0).unwrap();

    let request = system
        .request_affinity(thread.id(), singleton_affinity(2, 1))
        .unwrap();
    system.drain_owner_control_at(cpu0.as_mut(), 1).unwrap();
    system.mark_exited(thread.id()).unwrap();

    assert_eq!(
        request.try_result(),
        Some(Err(ax_task::TaskError::StaleThreadId))
    );
    system.drain_owner_control_at(cpu1.as_mut(), 2).unwrap();
}

#[test]
fn exited_thread_waits_for_in_flight_migration_delivery() {
    let (system, mut cpu0, mut cpu1, _idle1) = online_pair();
    let thread = ready_thread(&system, SchedulePolicy::default());
    let thread_id = thread.id();
    system.enqueue_at(cpu0.as_mut(), thread_id, 0).unwrap();

    system
        .set_affinity(thread_id, singleton_affinity(2, 1))
        .unwrap();
    system.drain_owner_control_at(cpu0.as_mut(), 1).unwrap();
    system.mark_exited(thread_id).unwrap();
    drop(thread);
    assert_eq!(
        system
            .service_deferred_task_work(ax_task::DEFAULT_BATCH_LIMIT)
            .unwrap()
            .processed(),
        0,
        "an inbox-held migration delivery must pin registry-owned resources"
    );

    system.drain_owner_control_at(cpu1.as_mut(), 2).unwrap();
    assert_eq!(cpu1.queued_summary(), 0);
    assert_eq!(
        system
            .service_deferred_task_work(ax_task::DEFAULT_BATCH_LIMIT)
            .unwrap()
            .processed(),
        1
    );
    assert_eq!(
        system.thread_state(thread_id),
        Err(ax_task::TaskError::StaleThreadId)
    );
}

fn online_pair() -> (
    TaskSystem,
    core::pin::Pin<Box<ax_task::CpuLocal>>,
    core::pin::Pin<Box<ax_task::CpuLocal>>,
    ax_task::ThreadHandle,
) {
    support::clear_handles();
    let system = TaskSystem::new(TaskSystemConfig::new(2)).unwrap();
    let mut cpu0 = system.create_cpu_local(CpuId::new(0)).unwrap();
    let mut cpu1 = system.create_cpu_local(CpuId::new(1)).unwrap();
    system
        .register_idle_thread(
            cpu0.as_mut(),
            ThreadSpec::new(SchedulePolicy::fair(Nice::ZERO, FairMode::Idle)),
        )
        .unwrap();
    let idle1 = system
        .register_idle_thread(
            cpu1.as_mut(),
            ThreadSpec::new(SchedulePolicy::fair(Nice::ZERO, FairMode::Idle)),
        )
        .unwrap();
    system.bring_cpu_online(cpu0.as_mut()).unwrap();
    system.bring_cpu_online(cpu1.as_mut()).unwrap();
    (system, cpu0, cpu1, idle1)
}

fn online_triple() -> (
    TaskSystem,
    Vec<core::pin::Pin<Box<ax_task::CpuLocal>>>,
    ax_task::ThreadHandle,
) {
    support::clear_handles();
    let system = TaskSystem::new(TaskSystemConfig::new(3)).unwrap();
    let mut cpus = (0..3)
        .map(|cpu| system.create_cpu_local(CpuId::new(cpu)).unwrap())
        .collect::<Vec<_>>();
    let mut idle2 = None;
    for (index, cpu) in cpus.iter_mut().enumerate() {
        let idle = system
            .register_idle_thread(
                cpu.as_mut(),
                ThreadSpec::new(SchedulePolicy::fair(Nice::ZERO, FairMode::Idle)),
            )
            .unwrap();
        if index == 2 {
            idle2 = Some(idle);
        }
        system.bring_cpu_online(cpu.as_mut()).unwrap();
    }
    (system, cpus, idle2.unwrap())
}

fn ready_thread(system: &TaskSystem, policy: SchedulePolicy) -> ax_task::ThreadHandle {
    let thread = system.create_thread(ThreadSpec::new(policy)).unwrap();
    system.make_ready(thread.id()).unwrap();
    thread
}

fn singleton_affinity(cpu_count: usize, cpu: u32) -> CpuSet {
    let mut affinity = CpuSet::empty(cpu_count);
    affinity.insert(CpuId::new(cpu));
    affinity
}
