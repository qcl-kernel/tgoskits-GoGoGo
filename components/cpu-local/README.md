...
| RISC-V | prefix recovery or `sscratch` | `tp` without TLS; CPU runtime anchor with TLS | current-thread header | `tp=TLS`, `sscratch=CPU base` |
| LoongArch64 | r21, mirrored in KS3 | `tp` without TLS; CPU runtime anchor with TLS | current-thread header | `tp=TLS` |
