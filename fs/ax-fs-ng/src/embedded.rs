...
impl FilesystemOps for EmbeddedFilesystem {
    ...
    fn root_dir(&self) -> DirEntry {
        self.root
            .get()
            .expect("embedded root not initialized")
            .clone()
    }
...
impl DirNodeOps for EmbeddedDir {
    fn read_dir(&self, offset: u64, sink: &mut dyn DirEntrySink) -> VfsResult<usize> {
        ...
        let parent_inode = this
            .parent()
            .map_or(self.node.inode, |parent| parent.inode());
        ...
        let mut count = 0;
        for (index, (name, inode, node_type)) in
...
pub fn new_filesystem(archive: &'static [u8]) -> Result<Filesystem, EmbeddedArchiveError> {
