...
/// Sets an interface's administrative MTU (`SIOCSIFMTU`).
///
/// `mtu` must already be validated against the device-kind bounds by the caller
/// (Linux gates this in `dev_set_mtu` against `dev->min_mtu`/`dev->max_mtu`).
///
/// # Data-plane scope
///
/// This records a per-interface MTU that `SIOCGIFMTU` reads back, matching the
/// Linux control-plane contract of an interface attribute userspace can set and
/// query. It deliberately does *not* retune the packet path: every interface
/// (loopback, ethernet, and all tun/tap) shares a single smoltcp [`Interface`]
/// fronted by one router `phy::Device` whose `max_transmission_unit` is the
/// stack-wide [`consts::STANDARD_MTU`], and the router/tun queue slots are
/// fixed-size inline arrays sized to that constant. Lowering the smoltcp MTU
/// here would shrink it for *every* interface at once, and raising it past the
/// slot size cannot be honored without reworking the whole data plane. So a
/// stored MTU below `STANDARD_MTU` still lets the stack emit up to
/// `STANDARD_MTU`-sized frames; segmentation to the smaller value is not yet
/// enforced on the wire. The value is authoritative for the control plane only.
pub fn set_interface_mtu(interface_id: InterfaceId, mtu: usize) -> AxResult {
    let mut service = acquire_service_for_config();
    service.set_interface_mtu(interface_id, mtu)
}
