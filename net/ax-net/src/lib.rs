...
/// # Data-plane scope
///
/// Records the per-interface MTU that `SIOCGIFMTU` reads back **and** pushes it
/// into the interface's data-plane `TunShared` frame ceiling. After a lowered
/// MTU, `push_rx` (write, fd->stack) and `TunDevice::send` /
/// `TunEthernetDriver::transmit` (read, stack->fd) drop frames exceeding the
/// configured MTU (accounting for the 14-byte Ethernet header on TAP), while the
/// shared smoltcp wire stays pinned at [`consts::STACK_MAX_MTU`] so *raising* the
/// MTU still carries a jumbo frame whole. Mirrors Linux, where `dev->mtu` is
/// per-`net_device`: `tun_get_user` silently drops an over-MTU inject (write
/// still reports the full length) and `tun_net_xmit` drops an over-MTU frame with
/// no errno. Kinds with no `/dev/net/tun` datapath (loopback, ethernet) keep the
/// control-plane-only record.
pub fn set_interface_mtu(interface_id: InterfaceId, mtu: usize) -> AxResult {
    ...
    if let Some(name) = name
        && let Some(shared) = tun_shared_by_name(&name)
    {
        shared.set_mtu(mtu);
    }
    Ok(())
