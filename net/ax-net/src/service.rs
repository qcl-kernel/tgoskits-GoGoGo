...
impl Service {
    ...
    /// Records a new administrative MTU for an interface (`SIOCSIFMTU`) and
    /// applies it to the data plane.
    ///
    /// The value has already been range-checked by the caller against the
    /// device-kind bounds (Linux `dev->min_mtu`/`dev->max_mtu`). It becomes the
    /// control-plane attribute that `SIOCGIFMTU` reports. The shared smoltcp wire
    /// is already provisioned at [`STACK_MAX_MTU`](crate::consts::STACK_MAX_MTU)
    /// (see [`Router::capabilities`](crate::router::Router)), so any value within
    /// the bounds is immediately carriable: the heap-backed queue slots are sized
    /// to the frame, so a datagram within the new MTU traverses TUN read/write
    /// instead of being dropped against a stale wire MTU.
    pub fn set_interface_mtu(&mut self, interface_id: InterfaceId, mtu: usize) -> AxResult {
        let mut state = self.control.state.write();
        let interface = state
            .interfaces
            .iter_mut()
