...
impl Service {
    ...
    /// Records a new administrative MTU (`SIOCSIFMTU`) on the control record and
    /// returns the interface name when the kind is TUN/TAP, so the caller can
    /// push the value into that interface's data-plane `TunShared`. Returns
    /// `None` for kinds with no `/dev/net/tun` datapath (loopback, ethernet).
    ///
    ...
    /// control-plane attribute that `SIOCGIFMTU` reports.
    pub fn set_interface_mtu(
        ...
        let interface = state
            ...
            .ok_or(AxError::NoSuchDevice)?;
        interface.mtu = mtu;
        Ok(interface.kind.is_tuntap().then(|| interface.name.clone()))
    }
