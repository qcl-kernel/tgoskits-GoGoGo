...
// SIOCSIFMTU bounds, matching Linux `tun_net_init` (tun.c:1375). `MIN_MTU`/
// `MAX_MTU` are 68/65535; the usable upper bound is `MAX_MTU - hard_header_len`,
// which is 0 for a layer-3 TUN and `ETH_HLEN` (14) for an Ethernet TAP.
const MTU_MIN: i32 = 68;
const MTU_MAX_TUN: i32 = 65535;
const ETH_HLEN: i32 = 14;
const MTU_MAX_TAP: i32 = MTU_MAX_TUN - ETH_HLEN;
...
impl FileLike for Socket {
    ...
    fn ioctl(&self, cmd: u32, arg: usize) -> AxResult<usize> {
        ...
        match cmd {
            ...
            SIOCSIFMTU => {
                // Validate against the device-kind bounds Linux installs in
                // `tun_net_init` (min 68; max 65535 for a header-less TUN, 65521
                // for an Ethernet TAP), then record the value so `SIOCGIFMTU`
                // reads it back. Ethernet/loopback interfaces here have no smaller
                // ceiling than a TUN, so they share the TUN upper bound.
                let info = read_ifreq_interface(arg)?;
                let requested = i32::from_ne_bytes(read_user_bytes::<4>(
                    (arg + IFREQ_DATA_OFFSET) as *const u8,
                )?);
                let max = match info.kind {
                    InterfaceKind::Tap => MTU_MAX_TAP,
                    _ => MTU_MAX_TUN,
                };
                if !(MTU_MIN..=max).contains(&requested) {
                    return Err(AxError::InvalidInput);
                }
                ax_net::set_interface_mtu(info.id, requested as usize)?;
            }
