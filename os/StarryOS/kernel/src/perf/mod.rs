...
impl PerfEvent {
    ...
    fn propagate_members(&self, on: bool) {
        // Collect live members (reaping any closed ones) under the `members`
        // lock, then release it BEFORE taking the per-member `event` locks:
        // `members` is a `SpinNoPreempt` (holds preemption off) while `event` is
        // a sleepable `Mutex`, so locking `event` under `members` would trip
        // `might_sleep`. Mirrors the collect-then-lock discipline in `read_group`.
        let live: Vec<Arc<PerfEvent>> = {
            ...
            members.retain(|w| match w.upgrade() {
                ...
                None => false, // reap a closed member
            });
            live
        };
        for m in &live {
