...
impl TunFile {
    ...
    fn set_iff_inner(&self, arg: usize) -> VfsResult<usize> {
        ...
        // Linux returns EEXIST when TUNSETIFF is issued on an already-attached fd
        // (`tun_chr_ioctl`: `ret = -EEXIST; if (tun) goto unlock`, tun.c). The
        // check runs under the lock, which is then released before any
