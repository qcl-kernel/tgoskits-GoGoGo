...
///
/// `spin::Once` guards the one-time table allocation. Each slot is `SpinNoIrq`
/// because it is written from the IPI handler on the sampled CPU and read later
/// from `cpuN/cache` sysfs reads; the `Vec<CacheLeaf>` is heap-backed (the
/// allocator is up by the time sysfs mounts).
static PER_CPU_CACHE_LEAVES: spin::Once<Vec<SpinNoIrq<Vec<CacheLeaf>>>> = spin::Once::new();
...
/// every secondary is driven through `ax_ipi::run_on_cpu` to read on that PE and
/// store into its own slot - the same cross-CPU mechanism `stop_machine` uses.
...
pub fn init_cpu_cache() {
    let total = cpu_num();
    let table =
        PER_CPU_CACHE_LEAVES.call_once(|| (0..total).map(|_| SpinNoIrq::new(Vec::new())).collect());
