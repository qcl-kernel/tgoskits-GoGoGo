...
use axtest::prelude::*;
use starry_vm::{
    VmError, VmIo, VmMutPtr, VmPtr, VmResult, vm_load, vm_load_until_nul, vm_read_slice,
    vm_write_slice,
};
...
fn starry_vm_pointer_and_error_mapping_rules_hold() {
    ...
    ax_assert_eq!(
        dangling.as_ptr().vm_read(&mut vm),
        Err(VmError::AccessDenied)
    );
    ax_assert_eq!(dangling.vm_write(&mut vm, 42), Err(VmError::AccessDenied));
...
fn starry_vm_slice_access_rejects_invalid_user_ranges() {
    ...
    ax_assert_eq!(
        vm_read_slice(&mut vm, core::ptr::null::<u8>(), &mut []),
        Ok(())
    );
}
...
fn starry_vm_alloc_helpers_validate_bad_inputs_before_copying() {
    ...
    let empty: Vec<u8> = vm_load(&mut vm, core::ptr::null::<u8>(), 0).unwrap();
