...
fn console_output_ownership_moves_after_runtime_and_tty_are_ready() {
    ...
    let start = bind
        .find("entry.backend.ensure_started()?")
        .expect("the selected UART runtime must start before console handover");
    let tty = bind
        .find("entry.tty.bind_to(proc)?")
        .expect("the controlling TTY must bind before output ownership moves");
    let activate = bind
        .find("entry.backend.runtime.activate_console_output()?")
        .expect("the runtime console route must activate only after TTY binding");
    let claim = bind
        .find("ax_runtime::hal::console::claim_runtime_output()")
        .expect("the boot console must relinquish output after runtime activation");
    assert!(start < tty && tty < activate && activate < claim);

    let bind = init
        .find("tty::bind_console_to")
        .expect("the init session must bind the runtime-backed console");
    let stdio = init.find("add_stdio").expect("stdio must be installed");
    let publish = init
        .find("add_task_to_table")
        .expect("the user task must be published");
    assert!(bind < stdio && stdio < publish);
}
