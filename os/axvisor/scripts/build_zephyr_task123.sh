...
for tool in curl sha256sum tar cmake ninja dtc readelf python3 uv; do
...
mkdir -p "$OUTPUT"
cp -- "$BUILD/zephyr/zephyr.bin" "$OUTPUT/zephyr.bin"
...
python3 - \
    "$OUTPUT/zephyr.bin" "$OUTPUT/zephyr.bin.build-meta.json" "$entry" \
...
$SCRIPT_DIR/zephyr_image_metadata.py write \
    --image "$OUTPUT/zephyr.bin" \
    --source "$OUTPUT/zephyr.bin.build-meta.json" \
    --output "$OUTPUT/zephyr.bin.meta.json"
