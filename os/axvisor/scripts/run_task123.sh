...
resolve_rootfs_image() {
    ...
    if [[ "${#rootfs_candidates[@]}" -eq 0 ]]; then
        # Pull directly into the runner's image directory: `image pull` places
        # managed rootfs images at <local-storage>/<image-name>/<image-name>,
        # so without an explicit --local-storage the image lands in the
        # axbuild default (tmp/axbuild/rootfs) and this find still sees
        # nothing on a fresh machine.
        run_timed "$TASK123_BUILD_TIMEOUT_S" image-pull \
            "$CARGO" xtask image pull --arch aarch64 --local-storage "$TGOS_IMAGE_LOCAL_STORAGE"
