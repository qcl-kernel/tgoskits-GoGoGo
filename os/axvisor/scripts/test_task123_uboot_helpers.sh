...
grep -Fxq \
    "success_regex = ['(?s:(?:(?:RTBENCH_END|RTBENCH_STABILITY_END) status=PASS.*TASK123_STARRY_RTBENCH_END status=PASS|TASK123_STARRY_RTBENCH_END status=PASS.*(?:RTBENCH_END|RTBENCH_STABILITY_END) status=PASS))']" \
    "$output"
