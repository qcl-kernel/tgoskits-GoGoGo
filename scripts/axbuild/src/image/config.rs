...
/// Config layout written by an older axbuild: the image tool persisted
/// `download_dir`/`extract_dir` instead of `local_storage` and could omit
/// `auto_sync_threshold`. Parse through this lenient form and migrate.
...
impl ImageConfig {
    ...
    /// Migrates a previously persisted config. The image tool historically
    /// persisted `download_dir`/`extract_dir` instead of `local_storage`; keep
    /// the historical `extract_dir` as local storage so a machine that
    /// already pulled images there keeps finding them.
    fn from_legacy(raw: LegacyImageConfig, base_dir: &Path) -> Self {
        Self {
            local_storage: raw
                .local_storage
                .or(raw.extract_dir)
                .unwrap_or_else(|| crate::context::axbuild_tmp_dir(base_dir).join("rootfs")),
            registry: raw.registry,
            auto_sync: raw.auto_sync,
            auto_sync_threshold: raw
                .auto_sync_threshold
                .unwrap_or(DEFAULT_AUTO_SYNC_THRESHOLD),
        }
    }
