        ...
        if let Some(gic) = gic.typed_mut::<arm_gic_driver::v2::Gic>() {
            ...
            gich.set_virtual_interrupt(
                ...
                VirtualInterruptConfig::hardware(
                    unsafe { IntId::raw(vector as _) },
                    physical_intid as u32,
                    0,
                    VirtualInterruptState::Pending,
                    true,
                ),
...
fn inject_interrupt_gic_v3(vector: usize, physical_intid: Option<usize>) {
    ...
    if let Some(physical_intid) = physical_intid {
        lr += ICH_LR_EL2::HW::SET + ICH_LR_EL2::PINTID.val(physical_intid as u64);
    }
